...
Semantic adoption is tracked orthogonally to these five scientific coordinates. Main through PR #375 / `4d70c17` inventories 59 score paths: 28 direct typed entrypoints, 27 wrapper-covered frozen kernels, 3 migration-required paths and 1 non-applicable utility. The noncrossing codec, synthetic threshold-profile reconstruction/shape/symmetry certificates, and exact fixed-K motif control covariance add reusable exact infrastructure but no production transfer result or empirical threshold invariant. The motif certificate removes the need to estimate control/control Gram, but supplies neither target/control covariance nor demonstrated variance reduction. Units, model space, implicit-root or annihilator response, size-map and dependency fields still belong in a second scorer contract. These labels guide future composability; they never demote historical evidence or change task permission.
...
1. build the augmented P250 radius-four/radius-five joint operator and ask whether any fixed map satisfies annihilation and extension together; preserve the current empty R2/R3 survivor intersection instead of selecting either by best p-value;
2. decompose the four independent P337 generations into first-birth, second-completion and `DIRECT_RANK2` recurrence components, with N130/N170 retained as a cross-lineage control and missing path fields marked `not_scoreable`;
3. prove arbitrary-HNF orbit-Hall saturation for P334's corrected base-exchange plus transverse-mark reservoir, or locate its next exact obstruction; pursue BA as a separate concordance injection and retire mark-only/base-only searches;
4. transport P333's exact signed connected-history nilpotent into a positive or physical Q-dependent transfer, keeping Gram compatibility and dimension velocity separate;
5. add the smallest generic-Q detach/loop-weight generator to the P321 crossed/trivial closure modules and test whether the affine Hom space collapses to one normalized line before naming a Jordan thermal matrix element;
6. implement one small invariant-first proof-carrying exclusion only after exact zeros, Hankel minors and trace identities have been exhausted; an undecided relaxation never blocks exploration;
...
7. replace the completed Gaussian/annulus rectangle with a maximin `AU-UA` or intermediate-state observation, and keep N112 C3, the triangular gauge-free top statistic, a stronger ordinary H4/H12 source and Issue-27 as parallel opportunities.
