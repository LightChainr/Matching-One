...
**三份后续评审已转为实际分析。** 0/1孔资料加尾界不能确定齐次U符号；全孔密度面连通核给出固定构型的精确有限源规律；复核m64反号并算出普通采样仅见一次rank1的必要预算已达10^14–10^19级，因此不启动该点粗采样。更强的全m>=64符号证书由并行执行分支交付，已接入，未重复归功。[本轮结果与范围](notes/reviews-response-20260831.md)
