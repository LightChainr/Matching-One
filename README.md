...
- [`docs/NEXT-TARGETS.md`](docs/NEXT-TARGETS.md) — eight highest-information moves and their exact decision outputs.
...
The stacked P334 line has now moved beyond a tiny oracle. A real N65 same-modulus 20k-per-orientation archive records `tau1`, the up-to-sign primitive first-born line `ell1`, `tau2` and a separate `DIRECT_RANK2` atom. An exact N13 multi-orbit census then shows that axis and diagonal line orbits both carry birth and exit flux; their opposite characters and opposite incidence imbalances reinforce the same `dA4/dp` direction, with exact signed shares about `75.574/24.426` at `p_ref`. This localizes the finite response across more than one line orbit without identifying a continuum field.

P337 gives that archive a finite Fourier basis. Conditional on the unmerged integral-saturation theorem, flat twists by any finite abelian group of order `n` satisfy `S_n=n^2 P0+n P1+P2`; `S2,S3` and normalization invert the entire unmarked rank source. The exact F3 representation is not three unrelated fields: `[H,A,D]` is the irreducible A4 standard triplet, restricting to one neutral C3 coordinate `u` and one charged real plane `(v,w)`. A deterministic basis shear closes the transport convention pathwise but adds no physical evidence. A sibling zero-sample reconstruction finds one 20/20-LOO-stable N65 balanced-H orientation crossing at `p=.573633326` with slope `.0602462` and root SE `.02281`; it is unresolved from `p_ref` and remains a frozen geometry selector.

Draft PR #267 now pushes the same archive one step further in [`results/f3-activation-flux/latest.md`](results/f3-activation-flux/latest.md). At the frozen crossing and at `p_ref`, the signed first-birth H coordinate and the `ell_birth`-weighted rank-one exit coordinate at `tau2` have opposite signs, so `birth-exit` reinforces rather than cancels; by `p=.65`, H nearly self-cancels while the fixed charged-plane projection is nonzero under a correlated plug-in Wald diagnostic. That change is descriptive, not verified C3 transport or an independent significance result. Full aligned-batch covariance and arbitrary-p boundary coefficients are retained. The current archive records when rank two completes but not a gauge-audited marked complement representative. A fresh child should therefore retain raw winding, section/basis, transporter and ambiguity metadata alongside `tau1,ell_birth,tau2,DIRECT_RANK2`; only then can a stabilizer-C3 completion character be tested instead of repeating the same H4 row.
...
The completed branch-only P250 Z5 cubic production adds a different charged readout rather than another vote. One million replicas preserve the two exact primitive connected neutral channels `113/122` and their conjugates, but the complete cubic vector is compatible with zero. The subsequent normalized-separation selector closes the entire R1-R4 local-annulus landing-H4 family: only `d=1` is resolved everywhere, so no radius supplies two usable separations. A new root-marked projective-leg operator then passes the 2k propagation gate at `d=1,2` (minimum pair z `5.53/2.90`; `d=3` unresolved) with a nondegenerate cubic interface. This selects a genuinely different observer. The next P250 move is an unchanged-operator fresh support-first block on `d=1,2`; phase and closure remain downstream of support, and the descriptive smoke cubics are not a selection statistic.

The ordinary H4/H12 lane has advanced only in acquisition design. Main PR #325 freezes opposite-alias N305/N325 pairs; a branch-only target-blind 20k/design variance pilot freezes 600M/design as its first production grid point. It contains no target score or production result, and its two-row/two-column fit has zero residual degrees of freedom, so any future `A12` is a covariance-aware contrast—not an omnibus fit, an H12-only identification or another charged-sector vote.
...
Semantic adoption is tracked orthogonally to these five scientific coordinates. Main through `84ec3f1` inventories 55 score paths: 24 direct typed entrypoints, 23 wrapper-covered frozen kernels, 7 migration-required paths and 1 non-applicable utility. The newest typed replays now cover the frozen prequential ledger, P231 C/Q/S diagnostic, v14 scalar-root projector and Issue43 secondary ledger. They bind descriptors, registered maps, stage ordering, covariance and chronology without adding samples, refits, scores or evidence rows: historical mismatch/excluded states stay excluded, reused stages do not become independent, and the H4-null root does not identify a continuum field. Units, model space, implicit-root or annihilator response, size-map and dependency fields still belong in a second scorer contract. These labels guide future composability; they never demote historical evidence or change task permission.
...
1. freeze a fresh N130 P334/P337 child carrying `tau1,ell_birth,tau2`, raw completing winding/lift, basis/orientation, transporter, ambiguity flag, `DIRECT_RANK2`, the full A4 triplet and joint boundary covariance; score the frozen H crossing/dual and a gauge-covariant stabilizer-C3 completion character together;
2. advance P250's selected projective-leg operator to one fresh `d=1,2` support-first block without changing the observer; only after support passes, score cubic closure and linear-to-cubic phase transport;
3. derive a genuinely independent source coordinate from cross-boundary connectivity `n12`, winding/seam character or K1/K2-conditioned Euler hazard, exact-gate it on a small annulus/torus system, and only then revisit coupling rank;
4. treat the ordinary P321 modulus row as completed compatibility; move to resolved homology, a typed seam or a second defect/Jordan modulus function while sharpening the `p_c +/- lambda L^-3/4` homology window;
5. replace finite-size literal-Jordan hunting with a typed multi-width gate on eigenvalue collision, positive-metric overlap approaching one and the Gram-Schmidt generalized-vector coupling; in parallel compute the small link-state Gram radical and cross-boundary tangent;
6. replace the completed Gaussian/annulus rectangle with a maximin `AU-UA` or intermediate-state observation, and freeze the threshold tail-flow direction for one held-out amplitude rather than resampling the same rectangles;
7. keep N112 C3, the triangular gauge-free top statistic, ordinary H4/H12 and Issue-27 as parallel opportunity lanes; defer norm-10 production until a target variance/runtime pilot changes its cost forecast. Descriptor migration remains support work, never a permission gate.
