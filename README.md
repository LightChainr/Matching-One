...
The main line now contains three complementary finite checks. PR #282 identifies `K_minus/K_plus` with the first and second essential-H1 births in the exact filtration oracle. PR #283 reconstructs `P0=1-F1`, `P1=F1-F2`, `P2=F2` and the mean rank-one lifetime from an existing rank archive. PRs #284/#285/#286/#287 extend the short-period quotient frontier through index 9: 68 HNF quotients and 5,372,118 filtrations have no birth, reflection, rank-sum, reconstruction or plateau-line failures. PR #286 first added cached/direct semantics on all 24 axis-L2 paths; PR #287 uses that cache to add all 13 index-9 quotients and 4,717,440 new paths. The cumulative archive has 12,636 subset states and 17,727,344 rank-one plateau steps; maximum observed saturation index remains one and no path shows index evolution. This is a strong finite frontier, not a theorem for index 10 and above or every degenerate quotient. The older production rank archive still lacks the plateau line `ell`, Smith saturation index `iota` and local birth mark, which is precisely why the next stream should retain them and reuse the exact cached oracle rather than reimplement topology.
...
The unmerged double-projector staircase sharpens the ordinary rank-plane question: the vacuum/KdV spin-4 direction lies on the Alexander-even line and has `A_top=0`, while the regular unlabeled `[2]` four-leg carrier also has zero ordinary overlap. Thermal `Q4 epsilon`/Jordan is the first listed candidate allowed by both selectors. “Allowed” is still not “coupled.” A later nine-geometry branch-only production rejects all of the frozen Q4, Q4-Jordan, pure-H4, affine-log-H4 and zero models for the global line response, and exact gate algebra shows that every connected root-independent insertion against an observer `f(q)`, `q in {-1,0,1}`, reduces to contact moments. The observed global-line response is therefore a projective/contact control, not an independent local-field matrix element. Field-sensitive work must use variation within fixed-q sectors: separated landing marks, another bulk observer, a charged seam or a modular/stress readout.
...
The branch-only rank-birth algebra supplies a reusable local interface. The exact birth gates `I01` and `I12` give an even rank-increment row `S=I01+I12=M'` and an odd lifetime row `D=I12-I01=-partial_p P1`; every nonzero `D` insertion carries a canonical plateau line `ell`. Complement symmetry separates the normalized midpoint `C` as an odd clock translation from the gap `W` as an even rank-one lifetime. A common marked-birth pilot already stores these coordinates at q2 N65/N130 and P50 N145. Its mean `J_D4` q2 ratio is a noisy high-dimension lead, whereas the connected `A_top` row is now exactly classified as contact. The next bridge is therefore the existing-data clock/lifetime/shape decomposition, then a separated landing or independent-observer covector—not another global `qJ` replica.
...
1. reuse the K1/K2 and marked-birth archives to separate activation translation, curve deformation, clock/lifetime, line and landing coordinates; only then choose a mean-`J_D4` third point or separated-local production;
2. implement the tiny orientation-resolved Q=4 seam H4 kernel, whose frozen raw-numerator test separates singlet `1` from charged `[2,2]` `0` without an exponent fit;
3. form a common-source/readout Gaussian/annulus rectangle with both mixed orders `AU` and `UA`, while keeping static context interaction distinct from chronological memory;
4. measure a typed non-`A_top` or charged complex C3 response and then reuse its basis for the primitive Z5 fusion channels;
5. obtain the two external Taylor ratios required by the main-integrated rank-four mixing audit, through continuum dynamics or frozen train/held-out geometries;
6. turn the triangular log-pair control into a gauge-invariant cross-microscopic H4/E6/top-field statistic and calibrate the complete Q-tangent interface in parallel;
7. move relative-source theory from its rigid three-sector scalar algebra to a connectivity/defect radical, while exact factor-four and improved-action sprints proceed independently.
