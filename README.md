...
**当前唯一 P0 是 #275 的原始观测量、归一化与候选可识别性。** [C3 精确相位设计](experiments/p275-c3-phase-contract-20260901/REPORT.md)已完成 66 项 exact checks：共享复振幅时第二旋转可以分开 H4/H8，允许任意复增益时则对所有角度都不可识别；`7.5°` 在 signed-real gain 合同下可分，而 `15°` 有精确符号别名反例。下一步先证明 original trace/source/pooled-root `U` 属于哪种 amplitude/phase transport 合同，再用已有 covariance 做一次冻结判决；不先申请新角度或新样本。[执行与停止规则](notes/p275-observable-identifiability-gate-20260901.md)

**#537 已降为 P1，并完成一次范围受限的前瞻验证。** N25 one-defect 与 distance-two literal witnesses 否定 automatic two-independent-defect/six-arm gain；held-out N65 20M block 又在冻结的 canonical selected-carrier 分账中复现 `[-,-;-,+]` 和严格负的 `Delta`。[N65 结果与审计](results/p537-contact-stage-n65/REPORT.md)同时保留完整协方差、positive exposure 与 selected total。该分账不是坐标无关算符，`theta=-1` 也只是符号象限恒等。对同一数据块的[完整 full-T secondary](results/p537-full-t-transport/REPORT.md)通过 C4/kernel/归一化审计，给 `J65/J25=0.29396±0.03361` 的有限收缩；两点指数、CFT 标签和 N145 外推仍不进入 canonical 主张。#537 后续只接受 remainder transport 的解析证明或反例，不再用新尺寸拟合替代。
