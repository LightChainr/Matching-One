...
The main line contains three complementary finite checks. PR #282 identifies `K_minus/K_plus` with the first and second essential-H1 births; PR #283 reconstructs `P0/P1/P2` and the mean rank-one lifetime; PRs #284-#287 exhaust the short-period quotient frontier through index 9. Open PR #292 advances the same cached/subset-DP semantics through index 10: 86 HNF representatives, 70,690,518 filtrations, 31,068 cached subsets and 262,901,744 rank-one plateau steps have no birth, reflection, rank-sum, reconstruction or line failures; maximum `iota` remains one with no path evolution. This is an `open_pr` finite frontier, not yet `main` and not a theorem for index 11 and above or every degenerate quotient. The older production rank archive still lacks the plateau line `ell`, Smith saturation index `iota` and local birth mark.
...
2. implement the tiny Q=4 seam kernel with calibrated multi-orbit phases or typed internal edge data, then keep its spatial-harmonic certificate separate from the frozen colour-numerator test `1/0`;
