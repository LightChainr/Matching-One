...
The stacked P334 line has now moved beyond a tiny oracle. A real N65 same-modulus 20k-per-orientation archive records `tau1`, the up-to-sign primitive first-born line `ell1`, `tau2` and a separate `DIRECT_RANK2` atom, and reconstructs K1/K2, the archived primitive-line character and `dA4/dp=j4_birth1-j4_exit2` with full aligned-batch covariance. A post-hoc conditional composition score on that same block finds an exploratory same-modulus line-sorting contrast `.0117153+/-.0043509`; it is one coordinate of the same 20k dependency group, not an independent detection or a field identity. The immediate discriminator is therefore a prereveal fresh child that jointly carries birth/exit composition and projective-line characters, not another unmarked rank curve.

P337 gives that child a finite Fourier basis. Conditional on the unmerged integral-saturation theorem, flat twists by any finite abelian group of order `n` satisfy `S_n=n^2 P0+n P1+P2`; `S2,S3` and normalization invert the entire unmarked rank source. Individual nonzero `F_q` twists instead resolve the primitive winding line modulo `q`. Reanalysis of the same N65 archive shows that F3 is the first useful full D4 projective basis: its balanced H4 row is statistically tied with raw `chi4`, while an exploratory diagonal-reflection-odd row is sharper but belongs to a different sector. This is an exact cohomology transform plus a pilot sector observation—not yet a Potts defect, affine-TL module or local CFT-field identification.
...
The completed branch-only P250 Z5 cubic production adds a different charged readout rather than another vote. One million replicas preserve the two exact primitive connected neutral channels `113/122` and their conjugates in one 24-real covariance block. The zero-parameter common-phase closure survives (`.8396/2`, `p=.657`), and H12/H8/H4 all survive their conditional GLS rows, but the complete eight-real cubic vector is itself compatible with zero (`8.564/8`, `p~.380`). Consequently the nominal H12-first order neither confirms a field nor overturns the earlier linear charged `71/21/8` H8 tendency. The next useful acquisition must first establish gauge-normalized nonzero cubic support with neutral two-point factors and at least two separations; adding replicas to the same compact three-anchor row or adding a fourth phase model would not resolve the alias.
...
The first positive escape is an external bulk Euler observer `O_ext=V-E+F0`. Its far-from-root `J_D4` coupling is enormous at N325/N425 and survives removal of the radius-2 contact region, proving a bulk Euler-to-rank-birth response outside the q-only algebra. The preregistered stored-data continuation now also survives direct source-plane projection: `J_D_perp=J_D-beta J_S` retains 99.8792% and 99.9144% of the far response magnitude, with transfer amplitude `1.054088+/-.001956`. Thus the coupling contains a source component independent of direct `J_S` projection in the recorded lattice Gram metric. Branch head `9f2033a` now supplies the missing exact radius-6 axis/diagonal observer gate, an implemented 2x2 `[O_far,O_sep4] x [J_D_perp,J_S]` scorer and unresolved 20k engineering smoke. The frozen N325/N425 2M blocks and joint four-real-degree determinant score are not acquired. Exact observer rank is not coupling rank; no Q4 field or exponent is identified.
...
Semantic adoption is tracked orthogonally to these five scientific coordinates. PRs #345 and #346 add post-score P50 S-prime and P48 new-geometry identity wrappers without changing data, scores or evidence roles. Their independently merged inventory snapshots were temporarily stale; PR #347 now composes the live `main@42c1563` corpus to 44 paths: 13 direct typed, 12 wrapper-covered, 18 migration-required and 1 non-applicable utility. This changed no scorer or scientific conclusion. Units, model space, size-map, chronology and dependency fields still belong in a second scorer contract. These labels guide future composability; they never demote historical evidence or change task permission.

PR #339 adds the exact odd-jet ratios `kappa_r=d_r/d_1^r`, their sparse Jacobian and delta-method covariance propagation. This removes only a linear thermal-coordinate metric. It has a synthetic fixture rather than a data estimate, and it does not remove observable normalization or nonlinear reparameterization; its next useful role is a typed shared-observable adapter with delete-one recomputation, not a new H4/Jordan claim.
...
The next operator assignment is organized as three independent tomography axes: the two-observer determinant tests **source rank**; the P321 `E4(i rho)` curve tests **modulus shape**; and calibrated Q4/Z5 seams test **typed charge-spatial selection**. Agreement sharpens an assignment, while disagreement localizes mixing. These axes are not additive votes, especially when they reuse one dependency group or live in different observer sectors.

## Default attention

The current default order is:

1. freeze a fresh N130-child P334/P337 boundary-by-F3-character block, preserving full cross-boundary/cross-character covariance, and acquire the already frozen N325/N425 two-observer determinant; these independently localize activation mechanism and source rank;
2. treat the ordinary P321 modulus row as completed conditional compatibility; move to a resolved-homology/typed-seam or independent defect/Jordan projector row while reviewing transparency and the `p_c +/- lambda L^-3/4` homology window;
3. strengthen the N112 primitive C3 residual or add a nondegenerate geometry covector, keep the exact Q4 seam as the typed charge-spatial axis, and move Q-tangent work onto the complete P333 lift/projector/field descriptor;
4. replace the completed same-semantics Gaussian/annulus rectangle with a maximin `AU-UA` or intermediate-state observation capable of seeing chronology;
5. freeze the observed threshold tail-flow direction, test its amplitude on a fresh lineage and connect it to the thermal jet;
6. replace the now-completed compact Z5 reveal with a two-point-normalized, separation-aware linear-cubic bridge that first detects nonzero support; keep the gauge-invariant triangular top-field and ordinary H4/H12 contrast as parallel typed controls, but defer norm-10 production until a target variance/runtime gate changes its cost forecast;
7. migrate semantic descriptors at live evidence hubs, obtain the two external Taylor ratios for the rank-four mixing audit, and move relative-source theory into the connectivity/defect radical; score Issue-27 widths 22-24 opportunistically when the primary target arrives. Bounded PSLQ remains a low-cost side sprint, not the threshold mainline.
