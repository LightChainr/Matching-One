...
The stacked P334 oracle makes the missing directional state more concrete. Conditional on the integral-saturation parent, a rank-one plateau carries an up-to-sign primitive first-born line `ell1`, while a direct 0-to-2 jump is a separate typed atom. The exact identity `dA4/dp = j4_birth1-j4_exit2` turns the next H4 question into first-birth influx versus second-activation exit flux. Tiny controls cannot distinguish spin because their support lines are quarter-turn related. The high-gain next exact experiment is one modest quotient with multiple inequivalent D4 primitive-line orbits—not a larger unmarked rank archive.

Another branch-only bridge identifies one exact finite projector family at Q=1: up to the positive state-sum normalization, the graph polynomial, matching curve and homology balance are all `P2-P0`. The TL open/closed crossing is its controlled infinite-length endpoint, not a finite-rectangle identity. A child oracle makes a sharper conditional prediction: if the homology restriction is Virasoro-transparent and the moving field is ordinary thermal `Q4 epsilon`, then `C_width(rho)/C_width(infinity)=E4(i rho)`, with `C_width=C_N/rho^2`. A defect/contact term or logarithmic partner adds another modulus function. The N144 20k branch now demonstrates that the five shape roots require their full shared covariance; it has only one scale and no E4 score. A direct child freezes a fresh 100k x three-scale x five-shape scale-then-E4 decision with no pilot pooling, refit or optional stopping. It is ready to acquire but not started; the hash-pinned multiscale planning score is not contained in the branch and is not scientific evidence.
...
Semantic adoption is tracked orthogonally to these five scientific coordinates. Through merged PR #344, all 42 scorer paths on `main` have an audit class: 20 are migration-required, 11 direct typed, 10 wrapper-covered and 1 non-applicable utility. PRs #335/#336 type matching-odd and chronological P48; PR #338 hardens the inventory; PR #341 types the existing C4 tangent holdout; and PR #344 types both frozen Issue43 full-curve kernels. These migrations preserve data, block selection, frozen chronology, numbers and evidence roles. Units, model space, size-map, chronology and dependency fields still belong in a second scorer contract. Open PR #345 is a post-score semantic overlay around an existing prospective P50 S-prime result; until merge its projected 43/12/11/19 inventory is not main state. These labels guide future composability; they never demote historical evidence or change task permission.
...
1. run one multi-orbit P334 birth/exit-flux exact discriminator and acquire the already frozen N325/N425 two-observer determinant; these independently localize activation mechanism and source rank;
2. acquire and score the frozen fresh P321 three-scale/five-shape contract once, while reviewing its integral-saturation dependency and deriving the `p_c +/- lambda L^-3/4` homology window;
...
6. reveal the primitive Z5 `113/122` closure before phase ranking; keep the gauge-invariant triangular top-field and ordinary H4/H12 contrast as parallel typed controls, but defer norm-10 production until a target variance/runtime gate changes its cost forecast;
