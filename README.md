...
**#537 is the sole P0 theory task; P0 random production remains empty.** The requested independent tests are complete and their failed parameterizations are stopped. The broader #154/#334/#337 questions remain open at P1; #539 is P2 reproducibility support, not an execution queue.
...
- **#537, theory only:** the [exact thermal-gate audit](https://github.com/LightChainr/Matching-One/blob/2785e3bba2eb6a70fd4f2d390d3b3c2701a99d85/notes/p337-thermal-pivotal-gate-audit.md) proves that `d_p Cov(O,a)` has both kernel-reconnection and original rank/readout-pivotal channels. Bell-8 checks 4,140 partitions and 64,954 joins; N13 checks 1,198,080 state-edge-pair records. Omitting the readout term reverses the finite N9/N10/N13 sign. The same note now rewrites `J_N/A_N` exactly as a root-conditioned mixed Hessian and signed Schur covariance. The first bounded falsifier is whether the ordinary four-arm landing transfer matrix is rank one after C4 and Schur projection; any nonzero `2 x 2` minor stops that cancellation route. No random block is authorized.
...
Drafts #530-#533 and support issue #539 are P2. At PR #533 head `a7680426768ca93ead1fca9446ae86ca1fc15ced`, the Bessel/capillary sign `J_1(c)=I_0(2c)^2-I_1(2c)^2>0` is a C1 local/conditional sublemma. The proposed full root theorem remains P2/C0: its dependency commits are absent from the delivered ancestry, the bounded-root chart drops an `a*s` term, the full-lattice connected-polymer remainder is not uniform, and the adjacent-hole activity order should be `O(N a^3)`, not `O(N a^4)`.
