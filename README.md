...
**当前只保留两个P0决策实验：#154前瞻性时序传递、#334独立群体干预。** 已有archive分析封存为探索结果；新P0必须在采样前给出失败后会停止的机制线。预测与停线判据尚在定稿，未冻结前不采新随机块。当前科学判断以[STATUS](docs/STATUS.md)为唯一入口，[NEXT-TARGETS](docs/NEXT-TARGETS.md)只列执行顺序。[清理记录](docs/REPOSITORY-TRIAGE-20260831.md)保存原13项结题及优先级操作；所有已有数据、分支和PR保留。
