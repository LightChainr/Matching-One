...
- [`notes/scientific-frontier-delta-20260830-post267.md`](notes/scientific-frontier-delta-20260830-post267.md) — post-snapshot P250/P333/P334/P337 mechanism changes.
...
Another branch-only bridge identifies one exact finite projector family at Q=1: up to the positive state-sum normalization, the graph polynomial, matching curve and homology balance are all `P2-P0`. The TL open/closed crossing is its controlled infinite-length endpoint, not a finite-rectangle identity. A child oracle makes a sharper conditional prediction: if the homology restriction is Virasoro-transparent and the moving field is ordinary thermal `Q4 epsilon`, then `C_width(rho)/C_width(infinity)=E4(i rho)`, with `C_width=C_N/rho^2`. The frozen fresh N144/576/1296, five-shape, 100k-per-shape campaign is now complete on branch `ca72994`: the fixed `N^-2+N^-3` scale gate passes (`2.00023/4`, `p=.7357`) and the conditional no-refit E4 residual is not rejected (`4.02746/3`, `p=.2585`). This is compatibility under a fixed three-scale parameterization, not an exponent measurement, a resolved amplitude ratio, projector transparency or a field identity. The pilot tension did not reproduce, so ordinary P2-P0 shape replication has low next value; the next row must resolve homology/sector, add a typed seam or expose a defect/Jordan second modulus function. The result also records a provenance exception: acquisition used `Huawei-CodeBuddy-TgFr7R`, not the currently designated TV2N0X environment, and remains branch-only pending chronology and infrastructure review.
...
The completed branch-only P250 chain has moved past both support-first and fitted spectra. Draft PR #267 still establishes one useful one-dimensional fact: a common T/A rank-three annihilator closes the old N505 axis window. An independent 80k/400-batch bivariate block preserves signed C4 but rejects common commuting diagonal ranks through three and the frozen Weyl/free-shared-eigenpair rank-five families. The model-free Hankel result at `a770ac9` now includes defective/Jordan commuting states: each plus/minus two-charge block rejects rank at most four and is compatible with rank at most five at the frozen `.01` level (`p=.0543/.0655`), while a raw four-channel shared rank at most five fails (`p=2.84e-5`). This is a truncated compatibility result, not exact rank five. A subsequent direction-only comparison at `a46ed63` leaves four Alexander reflection-conjugation maps and the identity comparators alive; because it compares extracted null directions without jointly enforcing both annihilation residuals, it neither identifies an Alexander bridge nor explains away the shared-rank rejection. The highest-information next object is a zero-sample candidate-constrained joint-null score on the same 400 batches. Only a surviving nonidentity map should promote the sector-bridge story; a radius-five shell comes next, and ordered `TxTy/TyTx` histories remain later because path order was never archived.
...
1. use the existing P250 400-batch covariance to jointly test each candidate sector map against both hand-specific annihilation residuals; do not promote the direction-only bridge comparison;
...
3. realize P334's unique global `D x F` critical pair across displacement classes and pursue BA as a separate concordance injection;
...
5. implement one small proof-carrying model exclusion with complete gauge/boundedness and exact or interval certificate reconstruction; an undecided relaxation never blocks exploration;
6. derive a genuinely independent source coordinate from cross-boundary connectivity `n12`, winding/seam character or K1/K2-conditioned Euler hazard, while moving P321 and Q4 work toward representation, sphere-fusion and modulus passport coordinates;
