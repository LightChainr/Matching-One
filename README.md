...
- [`notes/scientific-frontier-delta-20260830-late.md`](notes/scientific-frontier-delta-20260830-late.md) — late P250/P333/P334/P337/Issue55 result chains and the resulting team handoff.
...
The stacked P334 line has now moved beyond a tiny oracle. A real N65 same-modulus 20k-per-orientation archive records `tau1`, the up-to-sign primitive first-born line `ell1`, `tau2` and a separate `DIRECT_RANK2` atom. Exact N13/N17 censuses initially put about `76/24` of the signed response in the two line orbits near `p_ref`, but the later asymmetric-HNF atlas shows that this share and close root spacing are not laws. What survives is sharper: scalar source/sink currents determine their balance times, while the exact character Gram sign determines reinforcement versus cancellation. Every scanned torus row through HNF index 12 has one simple orbit root, and `branch_only` child `a9da7cd` proves that boundary association plus transport-moment domination on both complementary carriers implies ULC. The live theorem task is to derive those two moment inequalities from genuine homology geometry—or find a torus counterexample—not to resume an already-falsified global exchange proof.

P337 gives that archive a finite Fourier basis. Conditional on the unmerged integral-saturation theorem, flat twists by any finite abelian group of order `n` satisfy `S_n=n^2 P0+n P1+P2`; `S2,S3` and normalization invert the entire unmarked rank source. The exact F3 representation is one A4 triplet `[H,A,D]`, not three fields. On Gaussian-ideal quotients the unweighted A/D coordinates are exact quarter-turn-odd nulls; explicit sources `q_A=T_01-T_10` and `q_D=T_12-T_11` activate the missing B1/B2 sectors. The reused N65 response is concentrated in the A net current rather than common activity. Fresh independent N85 and held-out N145 blocks keep this natural current nonzero, and the exact `cos(4 theta)` geometry crosswalk trained on N65/N85 predicts the N145 vector with `chi2=1.519/2`; an added projective scalar improves the source fit by only `.121` and is not required.

Draft PR #267 pushes the same archive one step further in [`results/f3-activation-flux/latest.md`](results/f3-activation-flux/latest.md). At the frozen crossing and at `p_ref`, signed first birth and first-line exit reinforce H; by `p=.65`, H nearly self-cancels while the fixed charged-plane projection is nonzero under a correlated plug-in Wald diagnostic. That reuse is descriptive and shares the N65 dependency block. The higher-information acquisition is now a common enriched schema for the N130 exact-angle-flip child of N65 and the N170 exact-angle-flip child of N85: retain raw completion winding/lift, basis, transporter and ambiguity together with the full A4/F3 currents. N170 is preregistered at `cf1bdf8` but has no data, and its named server is outside the allowed TV2N0X environment; only the frozen scientific target is reusable until execution routing is corrected.
...
The completed branch-only P250 chain has already passed the former support-first milestone. A fresh 10k block keeps strong `d=1,2` pair propagation but does not reproduce the second-separation cubic, so the compact OPE/phase target is no longer the live question. An independent N505 80k stream resolves every pair row through `d=5` and rejects a single exponential plus all frozen one-component cylinder shapes. Covariance-aware reuse finds a minimal rank-two T-row recurrence that predicts held-out N505 `d=5` and the shorter N325 geometry, but exact `X=T+A`, `Y=T-A` tomography shows that even free A amplitudes at those roots fail the held-out direction row (`p=2.12e-10`). Draft PR #267 now completes the zero-sample common-annihilator gate on all eight T/A rows: common rank two fails (`74.788/28`, `p=3.88e-6`), while common rank three closes both its training window (`13.314/10`, `p=.207`) and strict `d=5` holdout (`13.205/16`, `p=.658`). This is one retrospective N505 dependency block and a finite-window state-count result, not three continuum fields. The next P250 acquisition should therefore realize the extra directional state with mixed displacements `T_x,T_y,R,D`, not return to scalar phase or compact-cubic voting. See [`results/p250-projective-leg-joint-ta-rank/latest.md`](results/p250-projective-leg-joint-ta-rank/latest.md).
...
Semantic adoption is tracked orthogonally to these five scientific coordinates. Main through PR #363 / `28133bc` inventories 56 score paths: 25 direct typed entrypoints, 24 wrapper-covered frozen kernels, 6 migration-required paths and 1 non-applicable utility. The threshold Krawtchouk wrapper and earlier prequential, P231, v14-root and Issue43 replays bind descriptors and chronology without adding samples, refits, scores or evidence rows. Units, model space, implicit-root or annihilator response, size-map and dependency fields still belong in a second scorer contract. These labels guide future composability; they never demote historical evidence or change task permission.
...
1. prepare one common N130/N170 enriched projective-birth schema carrying `tau1,ell_birth,tau2`, raw completing winding/lift, basis/orientation, transporter, ambiguity, `DIRECT_RANK2`, full A4/F3 currents and joint covariance; execute only on TV2N0X or an explicitly approved local route;
2. turn P250's finite-window rank-three closure into a mixed-displacement `T_x,T_y,R,D` vector-state gate, with both one word and one geometry held out;
3. prove BA/TM from real homology-carrier geometry—or find a torus counterexample—and place one transfer/partition-algebra generator on P333's now-classified dual-number radical;
4. derive a genuinely independent source coordinate from cross-boundary connectivity `n12`, winding/seam character or K1/K2-conditioned Euler hazard, while moving P321 from ordinary E4 compatibility to resolved homology, a typed seam or a second defect/Jordan modulus function;
5. replace the completed Gaussian/annulus rectangle with a maximin `AU-UA` or intermediate-state observation, and freeze the threshold tail-flow direction for one held-out amplitude rather than resampling the same rectangles;
6. build the complete measure + projector + explicit-field Q interface on a positive control, using the exact Q-velocity and Ward fingerprints rather than another raw scalar Q derivative;
7. keep N112 C3, the triangular gauge-free top statistic, an independently strengthened ordinary H4/H12 source and Issue-27 as parallel opportunity lanes. Descriptor migration and resource gates remain support work, never permission gates.
