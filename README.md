...
**当前唯一 P0 是 #275 的原始观测量、归一化与候选可识别性。** [C3 精确相位设计](experiments/p275-c3-phase-contract-20260901/REPORT.md)之后，2M paired N65 [finite primitive-C3 gate](results/p275-gaussian-c3-phase/REPORT.md)在冻结的 signed-real 两模型合同内强烈拒绝 pure H4（nominal `p=9.37e-18`），而未拒绝 H8 alias（`p=.292`），因此该子门停止且不补样。这不识别 continuum H8，也不适用于 original square-site `U`；P0 只剩实际 observable/normalizer map，不再申请新角度或样本。[执行与停止规则](notes/p275-observable-identifiability-gate-20260901.md)

**#537 已降为 P1，并完成一次范围受限的前瞻验证。** N25 one-defect 与 distance-two literal witnesses 否定 automatic two-independent-defect/six-arm gain；held-out N65 20M block 又在冻结的 canonical selected-carrier 分账中复现 `[-,-;-,+]` 和严格负的 `Delta`。[N65 结果与审计](results/p537-contact-stage-n65/REPORT.md)同时保留完整协方差与 exposure；selected total 是同块派生量。该分账不是坐标无关算符，`theta=-1` 也只是符号象限恒等。对同一数据块的[完整 full-T secondary](results/p537-full-t-transport/REPORT.md)给 `J65/J25=0.29396±0.03361` 的有限收缩。随后 N145 200M 冻结区间跨过决策边界，正式 verdict 为 `UNRESOLVED`、无 top-up；两点幂和 gauge-horizontal sidecar 只作 P2 fingerprint。#537 后续只接受 remainder transport 的解析证明或反例，不再用新尺寸拟合替代。
