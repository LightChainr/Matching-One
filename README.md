...
- [`notes/production-first-model-elimination-20260830.md`](notes/production-first-model-elimination-20260830.md) — the production-first loop and the first E_top/P250 model eliminations.
...
The project is no longer waiting for another semantic layer before using its machinery. Draft PR #267 now contains two production-facing, zero-sample model eliminations. The ten-size full-covariance rank-plane score excludes a pure Alexander-odd response (`E_top=0`), either activation carrying zero directional H4 response, one common `E_top=lambda A_top` line and one uncorrected `N^-13/8` amplitude. A separate 99% familywise no-pooling outer-set audit still excludes the three exact-zero lines but leaves a common ratio interval; that difference records proof strength rather than an evidence vote. In the charged P250 line, one augmented old-plus-degree-five Schur gate rejects all 16 frozen maps, and the plus-hand rank-at-most-five chart itself rejects. The live question is therefore larger state dimension versus ordered/path-enriched memory, not another R2/R3 election. See [`notes/production-first-model-elimination-20260830.md`](notes/production-first-model-elimination-20260830.md).
...
The stacked P334 line has now moved beyond a tiny oracle. A real N65 same-modulus 20k-per-orientation archive records `tau1`, the up-to-sign primitive first-born line `ell1`, `tau2` and a separate `DIRECT_RANK2` atom. Exact N13/N17 censuses initially put about `76/24` of the signed response in the two line orbits near `p_ref`, but the later asymmetric-HNF atlas shows that this share and close root spacing are not laws. What survives is sharper: scalar source/sink currents determine their balance times, while the exact character Gram sign determines reinforcement versus cancellation. The `branch_only` chain through `7ef99ae` proves that two-carrier BA plus aggregate TM implies ULC, makes aggregate TM imply every proper Hall cut, and reduces aggregate TM itself to the curvature-corrected Rayleigh inequality `4DF<=M^2+4Y(T-D)`. Later exact configuration gates at `c2d8170 -> 6a7b042` refute universal mark-only, union-preserving and base-only injections: after the fixed-line output-base correction, base-only reaches `588/1152` and transverse-only `768/1152` on the minimal N6 rows. One carrier-base exchange plus one independently released transverse output mark reaches `1152/1152` on every N6 row and `5760/5760` on the first Smith-(2,4) N8 gate. Full Hall is exactly equivalent to a factor-N compressed orbit-Hall problem for arbitrary HNF. The live theorem is now uniform Alexander saturation of this corrected reservoir—or the next exact obstruction—not another Hall atlas or a return to the refuted switches; BA remains a separate concordance injection.

A newer `branch_only` exact review at `fee3328` identifies a second, orthogonal issue. On the N10 Gaussian quotient, `(k,rank,line)` is not Markov under the actual uniform-permutation path law: two `K1` age classes have next-step exit hazards differing by exactly `1/57`. Adding `K1` gives an exact observed-history age augmentation. The same review identifies `DIRECT_RANK2` with the line-free diagonal event `K1=K2` and with the extra pathwise quadratic-variation mass; it is not a third projective-line H4 component. The conditional `D_N~N^-5/6` story still requires a six-arm bridge, universality and tail control. Existing P337 archives already contain the fields needed for the next zero-sample age/hazard/collision decomposition.

P337 gives that archive a finite Fourier basis. Conditional on the unmerged integral-saturation theorem, flat twists by any finite abelian group of order `n` satisfy `S_n=n^2 P0+n P1+P2`; `S2,S3` and normalization invert the entire unmarked rank source. The exact F3 representation is one A4 triplet `[H,A,D]`, not three fields. On Gaussian-ideal quotients the unweighted A/D coordinates are exact quarter-turn-odd nulls; explicit sources `q_A=T_01-T_10` and `q_D=T_12-T_11` activate the missing B1/B2 sectors. The reused N65 response is concentrated in the A net current rather than common activity. Fresh N85/N145 establish the geometry crosswalk; N170 overshoots the frozen H4 magnitude and N340 returns toward it. The preregistered N680 child lands closest to the two-mode forecast (`-.295` predictive SE), strongly excludes scale-neutral (`7.159` predictive SE) and leaves free-single unresolved (`.747` predictive SE). The four-generation fit does not identify a second eigenvalue. A later `branch_only` zero-production adversary at `efd6d31` leaves fixed identity dressing (`p=.669`), same-base rank-three Jordan (`p=.772`), free recurrence and free-single viable; N1360 is source-covariance limited across those mechanisms. The projective scalar remains null. The next reuse should locate the correction in first birth, second completion or `DIRECT_RANK2`; if the alias survives, use a geometry/modulus covector rather than manufacture a field name from one aggregate fit.

Draft PR #267 pushes the same archive one step further in [`results/f3-activation-flux/latest.md`](results/f3-activation-flux/latest.md). At the frozen crossing and at `p_ref`, signed first birth and first-line exit reinforce H; by `p=.65`, H nearly self-cancels while the fixed charged-plane projection is nonzero under a correlated plug-in Wald diagnostic. That reuse is descriptive and shares the N65 dependency block. N85/N170/N340/N680 retain a common `tau1,tau2,kind,ell_x,ell_y` archive schema, so the overshoot/return can now be decomposed into first birth, second completion and `DIRECT_RANK2` without new production. Existing N130/N170 archives also support an independent cross-lineage common-current check. Neither route can recover raw completion winding/lift, basis, transporter or ambiguity; those fields remain `not_scoreable`. N680 was acquired elsewhere on a historically recorded, non-permitted server; this refresh uses only the committed archive and does not contact that environment. No N1360 production is authorized or running.

Another branch-only bridge identifies one exact finite projector family at Q=1: up to the positive state-sum normalization, the graph polynomial, matching curve and homology balance are all `P2-P0`. The TL open/closed crossing is its controlled infinite-length endpoint, not a finite-rectangle identity. A child oracle makes a sharper conditional prediction: if the homology restriction is Virasoro-transparent and the moving field is ordinary thermal `Q4 epsilon`, then `C_width(rho)/C_width(infinity)=E4(i rho)`, with `C_width=C_N/rho^2`. The frozen fresh N144/576/1296, five-shape, 100k-per-shape campaign is now complete on branch `ca72994`: the fixed `N^-2+N^-3` scale gate passes (`2.00023/4`, `p=.7357`) and the conditional no-refit E4 residual is not rejected (`4.02746/3`, `p=.2585`). This is compatibility under a fixed three-scale parameterization, not an exponent measurement, a resolved amplitude ratio, projector transparency or a field identity. Three later exact theory slices name the missing structure. `c2a5e2d` shows that the leading E4 curve cannot by itself cancel the identity-dressed correction because the homology-resolved thermal one-point `F_t(tau)` remains unknown. `9dda24e` identifies the finite object as the two-closure functional `Tr(TA)-Q Tr(A)`, not one ordinary central element or one transparent crossed defect. `d0fca79` realizes the two closures as an exact graded direct sum, but its regular Q derivative is diagonal/semisimple; a Jordan limit needs an additional singular confluent map, and the affine intertwiner spaces have dimensions `2,3,9` at widths `2,3,4`, so no canonical nilpotent is selected. The highest-information next gate is therefore the smallest generic-Q detach/loop-weight generator and the induced affine-Hom dimension, not another ordinary P2-P0 rectangle. The result also records a provenance exception: acquisition used `Huawei-CodeBuddy-TgFr7R`, not the currently designated TV2N0X environment, and remains branch-only pending chronology and infrastructure review.
...
The first positive escape is an external bulk Euler observer `O_ext=V-E+F0`. Its far-from-root `J_D4` coupling is enormous at N325/N425 and survives both the radius-2 contact subtraction and direct source-plane projection. The frozen two-observer continuation is now complete: N325/N425 each have 2M paired replicas and 100 aligned batches, and the joint determinant score is `chi2_4=4.27897`, `p=.36957`. The current `[O_far,O_sep4] x [J_Dperp,J_S]` basis therefore does not reject one common projective lane. This is an informative null, not proof of one microscopic source and not a reason to repeat the same rows. The next source-rank experiment must add a mechanism-derived direction—such as cross-boundary connectivity fugacity, winding/seam character or endpoint-conditioned Euler hazard—rather than rotate or enlarge the existing matrix.
...
The exact CRT join says an unmarked final quotient does not remember the order of its factors. An exact path-filtration witness nevertheless shows that intermediate homology ranks can record when rank two first appears. A branch-only N650 map detects a large static mixed-factor interaction but saved no intermediate `h0,h2,h5,h25`, so it does not identify chronology. Chronological/morphism memory therefore belongs to an intermediate filtration, dynamic lineage or the two mixed orders `AU` and `UA`—not to the final endpoint alone.
...
Semantic adoption is tracked orthogonally to these five scientific coordinates. Main through PR #382 / `9572bcf` inventories 59 score paths: 28 direct typed entrypoints, 27 wrapper-covered frozen kernels, 3 migration-required paths and 1 non-applicable utility. PRs #376--#382 add exact rational/profile certificates, planar state operations, matching-pair exchange, a terminal-graph encoding and a synthetic rational `2 x 2` Jordan-geometry oracle. The degree-one height-100 result genuinely removes low-height rational threshold roots; the other additions are reusable exact infrastructure, not a production transfer result, physical Q-dependent Jordan realization or empirical threshold invariant. Draft PR #267 adds four channel-bearing scorers, giving a draft inventory of 63 = 28 direct + 27 covered + 7 migration + 1 utility; migration records future composability, not evidence quality. Units, model space, implicit-root or annihilator response, size-map and dependency fields still belong in a second scorer contract. These labels never demote historical evidence or change task permission.
...
1. reuse the existing P337/P334 birth archive to decompose the E_top residual into `K1`-conditioned age, line-weighted completion and line-free `DIRECT_RANK2` collision coordinates, preserving the full dependency covariance and using N130/N170 only as a cross-lineage control;
2. replace P250 fixed-map voting with one discriminator between a larger hand state space and ordered/path-enriched memory; the completed augmented gate already closes all 16 declared maps and the current plus-hand rank-at-most-five chart;
3. apply generic-Q weights and closure traces to the noncrossing detach/join primitives already on main, then recompute the P321 crossed/trivial affine Hom and first-jet Gram/radical gate before naming a Jordan matrix element;
4. prove arbitrary-HNF orbit-Hall saturation for P334's corrected base-exchange plus transverse-mark reservoir, or locate its next exact obstruction; pursue BA separately and keep the new age/collision result orthogonal to Hall;
5. transport P333's exact signed connected-history nilpotent into a positive or physical Q-dependent transfer, keeping Gram compatibility and dimension velocity separate;
6. use exact relations and full-covariance production distance as the default model-elimination ladder; escalate only surviving bounded classes to outward intervals or SOS, and let an inconclusive certificate leave exploration open;
7. derive a genuinely independent source coordinate from cross-boundary connectivity `n12`, winding/seam character or K1/K2-conditioned Euler hazard, while moving Q4 work toward representation, sphere-fusion and modulus passport coordinates;
8. keep the maximin `AU-UA` row, N112 C3, triangular gauge-free top statistic, stronger ordinary H4/H12 source and the separate degree-2--4 threshold track as parallel opportunities rather than prerequisites.
