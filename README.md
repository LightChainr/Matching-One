...
The main line contains three complementary finite checks. PR #282 identifies `K_minus/K_plus` with the first and second essential-H1 births; PR #283 reconstructs `P0/P1/P2` and the mean rank-one lifetime; PRs #284-#287, #292, #320 and #326 exhaust the short-period quotient frontier through index 12. The cached/subset-DP result covers 126 HNF representatives, 13,961,736,918 weighted filtrations, 170,332 cached subsets and 53,749,271,024 rank-one plateau steps with no birth, reflection, rank-sum, reconstruction or line failures; maximum `iota` remains one with no path evolution.

Open PR #330 extends the same finite locator through index 13: 140 HNFs, 101,140,028,118 weighted filtrations, 285,020 subsets and 500,805,335,024 plateau steps, still with all five failures zero and `iota=1`. This is inspectable finite counterexample-search evidence, not yet a `main` fact and not a second unrestricted theorem.

A newer `branch_only` four-sheet-cover argument claims the unrestricted theorem over rational homology, while honestly leaving integral image saturation unresolved. The exact next question is therefore sharper than “try another index”: prove that every rank-one component stabilizer is primitive, or let the finite oracle locate the least counterexample. A separate branch-only theorem proves that the honest-torus balance root `p_L` converges to square-site `p_c`; the open scaling question is now its near-critical window and rate. The older production rank archive still lacks the plateau line `ell`, Smith saturation index `iota` and local birth mark.
...
Semantic adoption is tracked orthogonally to these five scientific coordinates. After merged PR #332, all 37 scorer paths on `main` have an audit class: 25 are migration-required, 6 direct typed, 5 wrapper-covered and 1 non-applicable utility. The new identity gate wraps the existing Issue 50 N290 fixed-`p` score without new data, rescoring or H4/H12 evidence. Its remaining full-versus-half odd units, harmonic/model, size-map, chronology and dependency fields belong in a second scorer contract. These labels guide future composability; they never demote historical evidence or change task permission.

## Default attention

The current default order is:

1. build the two-observer coupling determinant for `O_far` plus a macroscopically separated calibrated local-H4/arm observer against `J_D_perp` and `J_S`; in parallel, connect the K1/K2 zero-area deformation to clock/lifetime coordinates;
2. review the unrestricted rational Digital Alexander proof and attack the one remaining integral stabilizer-primitivity gap; use the index-13 open frontier and later finite enumeration as counterexample locators, while deriving the `p_c +/- lambda L^-3/4` homology window;
3. strengthen the N112 primitive C3 residual or add a nondegenerate geometry covector; do not add N60-only samples;
...
6. reveal the primitive Z5 `113/122` closure before phase ranking; keep the frozen Q4 seam, gauge-invariant triangular top-field and complete Q-tangent interfaces as parallel typed controls;
7. migrate semantic descriptors at live evidence hubs, obtain the two external Taylor ratios for the rank-four mixing audit, and move relative-source theory into the connectivity/defect radical. Bounded PSLQ remains a low-cost side sprint, not the threshold mainline.
