...
Separate finite-field Rabin certificates already prove the corresponding integer polynomials irreducible over `Q`:
...
```text
7-cycle, 13-cycle, 23-cycle, 5-cycle, 13-cycle
```
...
For all five cases above, the certified prime cycle length `r` is larger than both `d` and `k` for every possible nontrivial divisor `d` of `n`:
...
Because its normal closure has Galois group `S_N`, and `S_N` is non-solvable for `N>=5`, the physical root is not expressible by radicals over `Q` in all five cases above.
...
2. the Galois group is the full symmetric group in every certified nontrivial case of degree at least 8/9.

This is stronger than merely observing complex roots or failing to find a low-degree factor.

## 6. What this does and does not say about the infinite threshold
...
- one additional primitive Gaussian orientation not equivalent to the axis/diamond sequences;
- an exact self-matching/solvable control where a persistent factor is guaranteed;
- modular cycle-type certificates at every newly reachable exact size.
