...
|---|---|---|---|
| P250 | The independent radius-five direction gate keeps R2, while the replay-verified radius-four joint-annihilation gate rejects R2 and keeps R3. Their frozen survivor sets have empty intersection. | Identity and every single fixed map as a currently identified explanation of both annihilation and extension. This does not exclude a context/modulus-dependent bridge, higher-degree state or general `5+5` direct sum. | Build one augmented joint operator over the old radius-four rows and already acquired degree-five shifts, preserving old/fresh jackknife influences separately; do not choose R2 or R3 by best p-value. |
| P321 | The ordinary `P2-P0` shape remains compatible with the frozen E4 curve, but exact follow-up work identifies the missing object as a two-closure trace. A graded direct sum realizes it, while a regular Q derivative is semisimple and does not select a unique Jordan extension. | Automatic cancellation of the identity-dressed correction; one ordinary central defect; Jordan mixing as a consequence of regular Q differentiation alone. | Add the smallest generic-Q detach/loop-weight generator and test whether the affine intertwiner space collapses to one normalized line. |
...
| P334 | Translation regularity collapses every proper Hall cut and aggregate TM is exactly `4DF<=M^2+4Y(T-D)`. Exact N6 gates then refute mark-only, union-preserving and base-only injections; one base exchange plus one free transverse mark saturates all minimal N6 rows and the first Smith-(2,4) N8 gate. Full Hall equals factor-N compressed orbit Hall for arbitrary HNF. | Larger Hall atlases, mark-only/base-only switching and union-preserving configuration crossovers. | Prove uniform arbitrary-HNF Alexander saturation of the corrected reservoir, or locate the next exact obstruction; pursue BA independently. |
...
jointly require `H_plus q=0` and `H_minus A(q)=0`.

That ambiguity was not left untouched.  The independently frozen radius-five
campaign at `11130aef3ee46df8d52e59c8bfe486eadcdbbaf5` adds 20 degree-five
shell points with 1,200,000 replicas and 400 batches.  The plus and minus
annihilators both extend (`p=1.0000`), while identity-plus-conjugation is
rejected (`p=.000728`).  Of the four Alexander reflection/conjugation maps,
only R2 survives the frozen `.01` rule (`p=.013379`); R3 lies just below it
at `.009686`.  This is a held-out degree-two annihilator-line morphism on the
available radius-five domain, not a complete transfer algebra, graph
isomorphism, exact state dimension or continuum assignment.  The stored
artifact names historical `Huawei-CodeBuddy-XPk2PZ`; this refresh reads only
the Git record and does not contact that non-designated environment.

The Draft candidate-constrained joint-null at
`99d23a705188ba9771523f1f10dc6d6bf9942ec4` now answers that audit.  The
mandatory identity-linear replay reproduces `a770ac9` with identical pivot and
residual, maximum covariance difference `3.71e-22`, 38 modes and p-value
difference `1.39e-19`.  In the primary family it rejects identity, R0, R1 and
R2 (`p=.0058241`) and retains only Alexander R3 (`p=.0774864`).  Across all
sixteen frozen maps only orientation-preserving R3 plus conjugation and
Alexander R3 plus conjugation survive.

The radius-five direction gate keeps only R2, while the radius-four joint gate
keeps only R3.  Their survivor sets are therefore disjoint.  The two p-values
must not be multiplied: the fresh 1.2M stream is independent, but the published
radius-five map score also reuses the old 80k moments.  The correct statement
is mechanism tension, not a winner.  The next zero-sample object is an
augmented candidate-constrained operator combining the old radius-four Hankel
rows with the already acquired degree-five shift rows, while retaining old and
fresh jackknife influence covariance separately.  Do not reopen a best-p vote.

## P321: the missing modulus object is a closure-module extension

The fresh three-scale rectangle result remains a compatibility statement for
the conditional E4 shape.  Three exact descendants now sharpen what must be
computed next.

- `c2a5e2d543cb6d402e27e6ea3e3814bd33208b8a` shows that the leading E4
  ratio does not automatically determine the identity-dressed correction:
  derivatives of the homology-resolved thermal one-point `F_t(tau)` remain.
- `9dda24ecccdc22dc73a3cbc55eb02d8236b15b6e` identifies the exact finite
  functional as `Tr(TA)-Q Tr(A)`.  Widths 3 and 4 reject its interpretation as
  one ordinary central element or one transparent crossed defect; width 2 is
  explicitly a degenerate false positive.
- `d0fca795c89338e16b732334c87d7885bc7fcfd0` realizes the crossed and
  trivial closures as an exact graded direct sum.  Its regular Q derivative
  is diagonal/idempotent, not nilpotent.  A Jordan confluence exists only after
  adding a singular intertwiner, and the affine solution spaces at widths
  2, 3 and 4 have dimensions 2, 3 and 9 rather than one.

The first live theory gate is therefore the smallest generic-Q detach or
loop-weight generator and the resulting affine-Hom dimension.  If it selects
one normalized line, a canonical Jordan thermal matrix element becomes a
prediction.  If it does not, the semisimple two-closure amplitude remains the
minimal model.  More ordinary rectangles cannot decide this extension class.
...
The exact configuration descendants `c2d8170 -> 6a7b042` then close the
naive route.  Universal mark-only and every union-preserving crossover fail
already at N6.  Enforcing the omitted fixed-line output-base condition reduces
the former base-only repair to `588/1152`; transverse-only reaches
`768/1152`.  Their conjunction—one carrier base exchange plus one free
transverse output mark—reaches `1152/1152` on every minimal N6 row and
`5760/5760` on the first Smith-(2,4) N8 gate.  Simultaneous translation acts
freely, so full Hall and orbit Hall are exactly equivalent with a factor-N
compression for arbitrary HNF.  The live theorem is uniform Alexander
saturation of this corrected reservoir, or the next exact obstruction.  BA
remains a separate marked-pair concordance problem.
...
1. Build P250's augmented radius-four/radius-five joint operator and test
   annihilation plus extension in one model.  Preserve the empty R2/R3
   survivor intersection until that calculation resolves it; do not reopen a
   best-p map vote.
2. Add the smallest generic-Q detach/loop-weight generator to P321 and test
   whether the crossed/trivial affine-Hom space collapses to one normalized
   line before naming a Jordan thermal matrix element.
3. Transport P333's exact signed connected-history nilpotent into a physical
   Q-dependent transfer expansion; keep positivity and dimension velocity as
   separate checks.
4. Prove arbitrary-HNF orbit-Hall saturation for P334's corrected base-exchange
   plus transverse-mark reservoir, or find its next exact counterexample;
   pursue BA independently and retire mark-only/base-only switching.
5. Use the committed N85/N170/N340/N680 birth archives to locate the P337
...
6. Use proof-carrying model elimination where it cheaply converts a declared
