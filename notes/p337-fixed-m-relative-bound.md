...
## 7. 2026-09-01：实际组件气体的进一步取舍

本段继续核对同一 `2690f665` 模型，不再构造摘要反例。得到两个实际模型恒等式/障碍，以及一个明确标明前提的几何比较；**固定 m 的 U 定理仍未完成**。没有新格点总体、随机块或云任务。

### 原组件表示的标准 KP 判据在 h=1 也无法统一成立

rank0 的占据组件 C 的裸活动度为 `z(C)=h^|C| m^(2-|partial C|)`；相邻、重叠及自重叠为不相容。考察明确的非负 Kotecký–Preiss 充分条件

```text
sum_(C' incompatible C) z(C') exp[a(C')] <= a(C),  a(C)>=0.
```

置 `x=h/m²`。C 内每个顶点的 singleton 均与 C 不相容，故条件强迫 `a(C)>=x|C|`。在原点 singleton 的左侧再保留含原点的 a×a 可缩方块一个单项，就至少为

```text
m² exp[a²(log h+h/m²)-4a log m].
```

只要 `log h+h/m²>0`，该下界随可嵌入方块增长而发散；特别 h=1 已失败。允许判据中的函数依赖体积也不能补救：singleton 的自不相容条件 `x exp[a({0})]<=a({0})` 要么本身不可能（x>1/e），要么把 `a({0})` 限制在仅依赖 x 的有限区间。

这排除了**这一裸组件表示中的标准非负 KP 方案**，包括任意非负 a(C)，不只 `a(C)=c|C|`。它不否定真实模型解析性、其他收敛方法或重整化两相轮廓。原先仅 h>1 的大岛活动度障碍因此得到加强；继续优化这套裸表示的短周长计数常数不能解决问题。

### rank2 的投影确实消去唯一绕行组件的颜色简并

不同黑 NN 组件在 torus 上不相交。两条线性独立的同调循环有非零交数，不能属于两个不同组件。因此总 rank2 必由一个 rank2 组件承载；其余组件只能 rank0。

令 Q=m²。局部颜色权重为 `h^K m^(-b) Q^C_B`；Sstar 的 rank2 投影 `m^-2=Q^-1` 精确将其改成

```text
h^K m^(-b) Q^(C_B-1).
```

在整数 Q，这逐配置等于固定唯一绕行组件的颜色，其他组件仍自由求和；任意实 m 则保留代数等式。它不只对 full ground state 成立，也没有证明 rank2 受限 partition 已完全由短轮廓描述。

### 等面积几何差可以怎样真正消去

**条件命题：**若实际相内模型已有共同的平移不变 connected-cluster 展开，锚定簇的绝对权重随支撑直径有统一 `C exp(-cR)` 尾，且大可缩轮廓/essential 网络的相对余项也受同样控制，那么两等面积 N、较小 systole 为 ell 的 torus 上，直径小于 ell 一个固定比例的簇可唯一提升到同一 Z² 对象。按同一根重数归一化后，每类都有相同的 N 个锚点，所以小簇对 log Z 的贡献逐项精确相等。仅大簇留在差中，给

```text
log Z_s,first - log Z_s,second = O(N exp(-c' ell)).
```

固定形状 dilation 下 ell~sqrt(N)，右侧趋0；这里没有人为切开 torus，也就没有额外的接缝 O(L) 项。

**尚未满足的前提**是实际共存窗口内的内外相受限 partition 比和统一大轮廓尾。占据岛的 `h^area` 必须由内部相熵补偿，不能直接设成1；同 pressure 名称、rank1 小或上面的颜色简并恒等式均不替代这一步。条件几何消去不能登记为本模型已证的相对误差，更不能登记为固定 m 的原 U 已证。
