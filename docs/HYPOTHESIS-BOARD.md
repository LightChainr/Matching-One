...
| H-SEAM-TYPED-SPIN4 | Ordinary singlet and four-leg charged spin-4 carriers can be separated by a colour seam before any exponent fit | Exact Q=4 2x2 preflight gives unnormalized character ratios 1 for `[4]` and 0 for `[2,2]`; the current log-v insertion is only a plumbing witness | Add one orientation-resolved H4 kernel and emit the six frozen raw `Z/N` values | Either identity numerator vanishes or the restored numerator ratios fail 1/0 after exact partition normalization | 2 |
...
| Typed exact spin-4 | Existing Q=4 seam preflight | Orientation-resolved H4 kernel on the tiny transfer system | H-SEAM-TYPED-SPIN4; supplies a positive Q-tangent control | low |
