...
1. **Run a live lifecycle drift pass at the start and end of each work block.** Compare open PR head/state with the registry and compare latest `origin/main` with the navigation base. Main is pinned here at `8ba849c`; the listed main integrations ending at PR #357 are integrated, while closed-unmerged #356 is absent and P321/P334/P337/P250/P333/two-observer/P158 remain separately classified branch-only objects. Treat drift as routine state change rather than wait for another cleanup.
...
4. **Treat semantic adoption as a composability layer, not a scientific grade.** Main now inventories 51 paths: 20 direct, 19 covered, 11 migrations and 1 utility. The new entrypoints replay frozen kernels without modifying their numerical results or evidence; they type the topology observable and registered affine map, not every derived response contract. Typed replay should preserve old numbers and never require new production.
...
| descriptor registry and scorer-adoption inventory | `main_integrated` through PR #357 / `8ba849c`; live 51/20/19/11 plus one utility and no new science; derived response contracts remain a second layer |
