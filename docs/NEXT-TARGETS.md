...
|---:|---|---|---|---|---|
| 1 | **Is the N65 line-sorting signal ordinary H4, modular reflection-odd structure, or a microscopic child-unstable effect?** | finite topology / birth geometric tilt+flat twist / boundary-conditioned line x F3 character / N130 Gaussian child / frozen fresh pilot | P334 has a real 20k path archive and exploratory conditional contrast `.0117153+/-.0043509`; P337 exactly resolves rank source and projective lines mod 3, but all current rows reuse one N65 block | One prereveal N130 child with `Delta_mu4`, full F3 character vector, raw chi4, direct-rank2 mass and common covariance | low/moderate fresh block; no repeat N65-only vote |
...
| 3 | **Why is ordinary P321 compatible with E4: transparent projector or an unresolved second modulus function?** | continuum module / thermal-Q4+projector/defect source / resolved homology or typed seam / equal-area rectangles+Q=4 torus / new typed row+exact | Fresh branch `ca72994` passes the fixed scale gate and does not reject E4, while neither amplitude nor transparency is resolved and the pilot tension vanished | One resolved-homology/typed-seam or independent defect/Jordan readout, plus a transparency proof/obstruction; no more ordinary P2-P0 replicas | low exact plus targeted typed acquisition |
...
| 5 | **Does the Z5 cubic observer have nonzero gauge-normalized support, and is its phase stable across separation?** | topology-typed module / deck source / normalized `113/122` linear-cubic bridge / N325 two separations / shared production | P250 1M closure survives, but the eight-real cubic vector is compatible with zero; all H12/H8/H4 models survive and cannot be ranked as fields | Record neutral two-point denominators and the linear charged row at two separations; test nonzero support before closure/phase transport | redesign readout, not more compact-row replicas |
...
1. **Run a live lifecycle drift pass at the start and end of each work block.** Compare open PR head/state with the registry and compare latest `origin/main` with the navigation base. Main is pinned here at `76ebad5`; #345/#346 are integrated, #347 is an open inventory-composition repair, while P321/P334/P337/P250/P333/two-observer/P158 remain separately classified branch-only objects. Treat drift as routine state change rather than wait for another cleanup.
...
4. **Treat semantic adoption as a composability layer, not a scientific grade.** Both P50/P48 wrappers are on main, but independent merges left the checked derived totals stale. Open #347 recomputes the live 44 paths as 13 direct, 12 covered, 18 migrations and 1 utility; it changes no scorer, score or evidence. Typed replay should preserve old numbers and never require new production.
...
|---|---|---|---|
| projective activation/twist flux | fresh N130 child of the N65 same-modulus pair | K1/K2, first-born line, direct-rank2 atom, conditional birth/exit composition, complete F3 characters and raw chi4 | ordinary H4 line sorting versus reflection-odd modular sector versus N65-only effect |
...
| charged complex geometry | completed N112/N60 phase work, P226 linear row and P250 compact cubic result | strengthen N112 in parallel; acquire two-point-normalized `113/122` at two separations with the linear row in the same stream | nonzero charged cubic support and stable single-eigenphase versus mixture/weak overlap |
...
- stacked `branch_only` P334 / `6f54935 -> 1714141 -> 381984d` adds the projective first-born line, a real N65 path archive, exact `birth1-exit2` reconstruction and an exploratory conditional line-sorting contrast, conditional on the unmerged integral-saturation parent.
...
The independent 20k block remains engineering-only. In the 1M/100-batch production, closure survives (`.8396/2`, `p=.657`) and H12/H8/H4 all survive; the complete eight-real cubic vector is nevertheless compatible with zero (`8.564/8`, `p~.380`). All shards, channels, controls and model rows form one dependency block. The next scorer must add `C0r-r` neutral two-point factors, `C000`, at least two separations and ideally the P226 linear charged row in the same stream. First establish gauge-normalized nonzero support, then test closure and linear-to-cubic phase transport.
...
| descriptor registry and scorer-adoption inventory | P50/P48 wrappers `main_integrated` through PR #346 / `76ebad5`; open composition repair PR #347 / `44ca017` recomputes live 44/13/12/18 plus one utility and no new science |
...
| primitive Z5 cubic production | `branch_only` result `analysis/p250-z5-charged-threepoint-20260830@be80f25`, `results/huawei-20260830/P250-z5-charged-threepoint/`; closure survives but cubic support is unresolved from zero |
