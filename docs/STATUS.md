...
**Stop:** no top-up, new lag/source, fifth contact feature or same-block model rescue follows from these results. #537 must first freeze the ordinary/no-extra landing semantics and then complete the surviving thermal/pivotal rate and pooled-root transport before any random work is considered; the separate fixed-m phase control remains distinct. The established C3 finite-size evidence remains valid without unique operator identification.
...
**The first finite rank test is complete.** Under an explicit six-block clean-two-bridge Bell-8 contract, the [N25 exact preflight](https://github.com/LightChainr/Matching-One/blob/ec3941b03b2694e827db1cba34766a82e6146a5a/experiments/p537-landing-matrix-preflight-20260901/REPORT.md) finds all six source/thermal row minors nonzero for each of the two nonzero C4 orbits and their sum. The summed P4/root minor is `-2.4843232721775393e-5`, the fixed-M Schur residual is `-4.217141611550048e-6`, and `T_p/M_p=+5.806332966676667e-6`; exact outward intervals fix all signs. A separate N13 rank-transition aggregate also has a nonzero minor and an exact zero readout/nonzero kernel `0->2` column. Eight `2^24` traversals reproduce every committed profile byte for byte with zero random samples and zero cloud jobs.

This rejects a universal finite thermal-rank-one cancellation and the stated minimal port-level contract. It does not yet settle the undefined phrase “ordinary four-arm/no-extra-branch,” because Bell keys do not encode off-port branching and the current K profiles do not retain the thermal site's before/after landing. #537 now has one bounded semantic action: freeze allowed Bell keys, a global branch flag, z-local transition rows and the C4 character, then test inclusion once. If the definition includes clean-two-bridge, the rank-one route is retired immediately; if it excludes it, only the minimally extended exact producer is rerun. No N expansion, random block or five/six-arm bound comes first.
