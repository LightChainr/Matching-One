...
**The thermal derivative shortcut is retired.** The [exact audit](https://github.com/LightChainr/Matching-One/blob/2785e3bba2eb6a70fd4f2d390d3b3c2701a99d85/notes/p337-thermal-pivotal-gate-audit.md) gives
...
The gate also has an exact compressed form. If `M(p(u,epsilon),epsilon)=u` and `Yhat(u,epsilon)=Y(p(u,epsilon),epsilon)`, then `partial_u partial_epsilon Yhat=T_N/D=J_N/A_N`. In logit coordinates this is the signed Schur covariance `T_t=<H,(a-Ea)S-(jM/M_t)B>_pool`, which keeps reconnection, readout and root/slope motion in one object. The next finite theory check is therefore the rank of the ordinary four-arm source/thermal landing matrix after C4 and Schur projection. A nonzero projected minor retires the proposed cancellation before any N25 extension; only rank-one survival permits a five/six-arm probability proof.

**Unmerged support does not change that boundary.** Drafts #530/#531 are P2 method support on the existing N25 exact block; #532 is a P2 finite-algebra candidate; #539 is P2 reproducibility support; #542 is a P2 analytic follow-up. At PR #533 head `b49e2a214c42ee746fb5174acc4028a761882d33`, the arbitrary-run determinant and `m_dir=1+sqrt(2)` threshold are C1 for the directed subfamily; the single-hairpin endpoint cancellation is C1 only under its stated reflection, uniform stripped-collar summability and `1/L` placement assumptions; and the four-word shortest L-triomino packet with `Delta g=6` and local residual `-8a+O(a^2)` is a finite local C1 calculation. The Ward coefficient is only canonical/leading-core at finite `a`, because the complementary packet weights have ratio `(1+a)^2/(1+a+a^2)`, not one. General physical collar/nonlocal packets, second thermal moments, moving root, original-U slope, `w>=2` and a uniform full-lattice remainder remain C0. Required ancestry (`2690f665`, `762dbaf4`, `410015f5`) is absent, the root chart still uses a non-uniform `a*s=o(Na^2)` step, one control character remains and one adjacent-hole term is still written `O(N a^4)` instead of `O(N a^3)`. None authorizes production or settles unrestricted fixed `m`.
