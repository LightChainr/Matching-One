...
**Designed and frozen, not yet run — `predictions/aspect_ladder_n1300_v2_20260905.yaml`.** The design is the *ladder* `r = 1, 2, 4` at one site count, not a single r=4 ratio, and it lives at **N=1300**, not the N=3380 this item named earlier.
...
Three orientations per family determine `C`, `A4` and `A8` together, so the spin-8 leakage is **removed rather than bounded** — the one caveat the N=290 run had to carry. The engine couples exactly two period matrices per run, so a family is **two paired runs** on disjoint seed streams rather than one three-orientation run: differencing within a pair kills `C`, two pairs determine `A4` and `A8`, and the third pair is their exact sum, available as an over-determination check but not part of the design. Six runs in all. That costs `Var(A4) = 0.2978` against `0.2719` for the N=290 estimator — 9 % more variance and one extra run per family buys the systematic outright.
