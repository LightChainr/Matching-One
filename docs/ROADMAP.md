...
The [exact gate audit](https://github.com/LightChainr/Matching-One/blob/2785e3bba2eb6a70fd4f2d390d3b3c2701a99d85/notes/p337-thermal-pivotal-gate-audit.md) has already resolved the finite semantics: `d_p Cov(O,a)` is the sum of kernel-reconnection and original rank/readout-pivotal channels. Bell-8 checks 4,140 partitions and 64,954 joins (`29,970` nonzero; `9,952` positive; `20,018` negative; `max |16 Delta g|=68`), and N13 checks 1,198,080 state-edge-pair records with 312 same-shared-count sign flips. In N9/N10/N13 controls the omitted readout term reverses the sign. The single-channel `partial_p E[g_xy]` shortcut is therefore retired.

The first live question is the full original-U quantity `T_N=jY_p-R*jM_p-R_p*jM`: prove `T_N=o(D/A_N)` and transport exact-`p_c` control to the pooled root. A naive absolute `R^4*pi_4(R)^3` shell bound needs `alpha_4>4/3`, beyond the currently available rigorous input of some `alpha_4>1`; #537 must supply stronger pivotal support, a signed cancellation/landing identity, or another normalization estimate. The second, separate fixed-m question still needs restricted-sector/phase control. The [actual component review](https://github.com/LightChainr/Matching-One/blob/baa5d33b2f87b2868aa0cb9d3f6518c93dbf3bff/notes/p337-fixed-m-relative-bound.md) already shows that improving the bare nonnegative KP counting constant is not the task. No source coefficient, distance grid, completion scan, N25 extension or random sample budget follows.

The full gate is now exactly `partial_u partial_epsilon Yhat=T_N/D=J_N/A_N` in the matching-mean coordinate, equivalently one signed Schur covariance in logit coordinates. First test the ordinary four-arm landing transfer matrix after C4 and Schur projection. Any nonzero `2 x 2` minor kills the thermal-rank-one cancellation route; only if all ordinary minors vanish and failures are confined to extra-arm/branch classes should work proceed to five/six-arm bounds and near-critical transport.

The post-cleanup Drafts #530-#533 and issues #539/#542 are P2 support/candidates, not competing priorities. #530/#531 provide oracle/method diagnostics on the existing exact N25 block; #532 preserves finite two-bridge algebra. At #533 head `b49e2a214c42ee746fb5174acc4028a761882d33`, the directed determinant, conditional reflection lemma and shortest local hairpin packet are C1 only within their explicit submodels. The full theorem remains P2/C0 because the physical collar, second thermal moment, moving root/original-U slope, `w>=2`, nonlocal west step and uniform full-lattice remainder are unresolved, alongside the recorded ancestry, `a*s`, control-character and `O(N a^3)` corrections. #542 is the bounded analytic/counterexample-search follow-up for exactly those gaps; it is not a production queue and does not displace #537's finite landing-matrix falsifier.
