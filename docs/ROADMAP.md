...
|---|---|---|
| activation flux | 1 | exact multi-orbit score and N65 boundary tomography are complete; fresh child adds the completing projective line and stabilizer-C3 character |
| source rank | 1 and 7 | current 2M matrix leaves rank one alive; exact-gate a physically new cross-boundary, winding or endpoint-hazard source |
| modulus shape | 2, 3, 4 and threshold-shape parallel line | ordinary P321 scale/E4 compatibility is scored; move to resolved-homology, typed seam or a second defect/Jordan modulus function |
| typed charge-spatial | 2, 4, 6 and 7 | P250 projective-leg `d=1,2` propagation is selected; freeze unchanged-operator support before phase |

### 1. Promote activation-boundary tomography and replace the exhausted source basis
...
**Completed lane:** freeze head `9f2033a` supplied the exact R=6 gate and result head `26cecb4` completed N325/N425 at 2M replicas and 100 aligned batches per size. The joint score `chi2_4=4.27897`, `p=.36957` does not reject rank one in the current observer/source basis. This is an informative null, not proof of one source. Do not repeat or reversibly rotate the matrix. The next source-rank row must be a new mechanism: cross-boundary connectivity `n12`, winding/seam character or K1/K2-conditioned Euler hazard, exact-gated before production.

P334 now supplies both an exact N13 multi-orbit census and a real N65 path archive. The N13 axis/diagonal orbits reinforce the same H4 derivative. P337 proves `[H,A,D]` is one A4 triplet, reconstructs complete F3 curves and closes deterministic basis-shear convention without physical evidence. PR #267's zero-sample flux tomography finds first birth minus the `ell_birth`-weighted rank-one exit at `tau2` reinforces H at the frozen crossing and `p_ref`; by `.65`, H nearly cancels while the fixed charged projection remains nonzero under a correlated plug-in Wald diagnostic. **Immediate output:** freeze one independent N130 child with `tau1,ell_birth,tau2`, raw completion winding/lift, basis/orientation, transporter, ambiguity flag, `DIRECT_RANK2`, full A4 covariance and the frozen H crossing/dual. Exact-gate a gauge-covariant stabilizer-C3 completion character before interpreting the marked complement representative. That missing gauge data—not more N65 precision—is decisive.

A larger-N copy of the same one-observer scalar row currently has lower information. The new source-rank tangent, activation completion character and charged seam are three distinct tests and must not be pooled.
...
Child `ca72994`, frozen at `8206087`, has now completed N144/576/1296 x five shapes x 100k. The fixed `N^-2+N^-3` scale law passes (`2.00023/4`, `p=.7357`), and the conditional no-refit E4 residual is not rejected (`4.02746/3`, `p=.2585`); the pilot tension does not reproduce. **Decision output:** ordinary scalar modulus-shape compatibility is complete. Do not spend the next block increasing the same P2-P0 shape precision. Use resolved homology, the orthogonal typed seam or an independent defect/Jordan projector to test transparency and a second modulus function. The result remains branch-only, and its recorded TgFr7R acquisition must be reconciled with the current TV2N0X isolation rule before integration. See `notes/p321-modulus-literature-brief.md`.
...
Main-integrated PR #291 supplies a fail-fast necessary gate: on one C4 direction orbit the nominal spin-4 character is constant and indistinguishable from a scalar; horizontal/vertical bonds are still one axial orbit. **Next decision output:** add calibrated multi-orbit phases under a declared shared two-component model, or provide typed/internal-edge transformation data. Axis+diagonal half-sum/difference is sufficient only after radial/type calibration and still does not exclude H12-like aliases. Emit constituent orbit numerators plus the six raw seam `Z/N` values, reporting spatial angular rank separately from colour character. This needs no exponent fit or large framework.

### 3. Move from the completed context rectangle to ordered composition
...
### 4. Resolve charged support before interpreting Z5 phase rank
...
The P250 one-million-replica cubic stream is complete but signal-limited. Its normalized-separation continuation then closes the R1-R4 local-annulus family. A root-marked projective-leg insertion changes the observer and passes its 2k propagation gate at `d=1,2` with minimum pair z `5.527/2.900`; `d=3` is unresolved. The cubic interface is nondegenerate, but no phase score was used and descriptive cubic p-values are not a promotion statistic.

**Next decision output:** strengthen N112 or choose a nondegenerate geometry covector in parallel, and freeze a fresh projective-leg `d=1,2` support-first block with unchanged operator semantics. Only after support passes should closure and linear-to-cubic phase transport be scored. Failure changes the insertion; it does not negate the whole charged sector.
...
For Jordan structure, replace literal finite-size block hunting with a typed multi-width triple: eigenvalue gap, positive-metric eigenvector overlap and Gram-Schmidt generalized-vector coupling. An emerging Jordan cell may appear only as width grows even when every finite transfer matrix is diagonalizable.

### 7. Build the connectivity radical and an exact improved-action family

The scalar relative source is exhausted: `q^3=q`, its three-sector algebra is semisimple and `HH^2=0`. More powers of the same source cannot add the missing state. Branch-only `5d8343d` now classifies the finite matching defect as a typed-site rank-image/site-Krushkal quotient and proves that the required four-terminal `J4` state is absent from ordinary product-local edge-subset polynomials under the declared contract. Main-integrated PR #329 supplies only the RGS/group-orbit canonical key for three- and four-terminal partitions; it is useful recurrence plumbing, not a gadget result or a discovered radical.

**Next decision output:** stop searching for a direct named edge-subset specialization; extend the canonical key with typed port and occupation labels, define the D4-preserving four-terminal junction composition table, derive a recurrence/coefficient algebra for the typed-site quotient, then quotient the scalar blocks and compute its radical/Jordan data. Project both the Q-tangent defect and the threshold success-to-failure vector into it; agreement would provide a shared backend, while separation would cleanly split the two theories.
...
Main also has a coherent 200-descriptor/952-map registry. Through `84ec3f1`, the live corpus contains 55 paths: 24 direct typed entrypoints, 23 wrapper-covered frozen kernels, 7 migrations and 1 utility. The newest replays add prequential, P231, v14 scalar-root and Issue43-secondary coverage without modifying any frozen kernel, numerical result or scientific conclusion. This is a semantic transport layer, not a new evidence grade; topology typing does not yet type every derived response. `Migration_required` does not invalidate a result.
...
Merged PR #341 types the C4 holdout, PR #344 types both Issue43 full-curve kernels, and later wrappers through PR #362 now include prequential, P231, v14 scalar-root and Issue43-secondary replay; none changes data, score or scientific priority. The prequential mismatch and Issue43 excluded states remain excluded, reused stages do not become independent, the P231 C-only view remains shared retrospective evidence and the v14 implicit root remains a response coordinate. The exact parametric, log-odds and Pell artifacts are bounded method or regression contracts rather than physical evidence.
...
- **Projective birth/twist flux:** exact N13 line orbits reinforce; P337 gives the A4 triplet and complete F3 curves; PR #267 splits N65 birth/exit flux and finds H reinforcement at the crossing/reference plus upper-p H cancellation. A fresh child with raw completion winding/lift, transporter and ambiguity metadata, followed by a gauge-covariant stabilizer-C3 character gate, is higher information than enlarging the old block.
- **P250 charged observer selection:** the R1-R4 local-annulus family is closed, while the projective-leg row passes `d=1,2` propagation. Preserve that operator for one fresh support-first block before any phase claim.
...
- **Semantic control plane:** main through `84ec3f1` inventories 55 paths: 24 direct, 23 covered, 7 migrations and 1 utility. PRs #339/#349/#354 are synthetic exact primitives and PR #351 is a bounded exact regression; PR #342 remains a resource gate, while PRs #359-#362 are frozen semantic replays. None is new scientific evidence. Migration changes future composability, not historical evidence role; the next layer must record units, model/harmonic, size map, chronology, response semantics and dependencies.
