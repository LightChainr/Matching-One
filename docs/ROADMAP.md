...
Stacked branch P334 now supplies a real N65, 20k-per-orientation path archive. It records `tau1,ell1,tau2,DIRECT_RANK2`, reconstructs K1/K2 and verifies `dA4/dp=j4_birth1-j4_exit2` with joint covariance. A post-hoc conditional composition finds an exploratory `.0117153+/-.0043509` same-modulus line-sorting contrast on that same block. P337 turns the projective mark into exact flat-twist tomography: aggregate order-2/3 twists recover the unmarked rank source, while individual F3 twists resolve axes, diagonals and reflection-odd projective rows modulo three. **Immediate output:** freeze one independent N130 child with boundary-conditioned birth/exit composition and the full F3 character vector. This distinguishes ordinary H4 sorting, modular reflection-odd structure and an N65-only microscopic effect without treating reused rows as votes.
...
The P250 one-million-replica cubic stream is now complete. Its `113/122` common-phase closure survives (`p=.657`) and all H12/H8/H4 conditional rows survive, but the eight-real cubic vector is compatible with zero (`p~.380`). The apparent H12-first order is therefore not a field identification and does not replace the distinct linear charged 71/21/8 block.

**Next decision output:** strengthen N112 or choose a nondegenerate geometry covector in parallel, but replace the compact cubic row with a common-stream two-point-normalized, two-separation linear-cubic bridge. First show nonzero gauge-normalized support; then ask whether closure and the linear-to-cubic phase transport are stable. Near-zero support means change the insertion, not add samples or phase templates.
...
Main also has a coherent 200-descriptor/952-map registry. PRs #345/#346 now contain both P50 S-prime and P48 new-geometry typed wrappers, but their independently merged checked totals are compositionally stale. Open PR #347 recomputes the live 44-path corpus as 13 direct, 12 covered, 18 migrations and 1 utility without changing any scorer or science. This is a semantic transport layer, not a new evidence grade. `Migration_required` does not invalidate a result.
...
Merged PR #341 types the C4 holdout, PR #344 types both Issue43 kernels, and PRs #345/#346 type the existing P50/P48 scorers; none changes data, score or scientific priority. The underlying model chronologies remain what they were, while these semantic gates are post-score and create no evidence votes. PR #347 is a derived-inventory composition repair only.
...
- **Projective birth/twist flux:** P334 has a real N65 joint archive and an exploratory conditional sorting contrast; P337 gives exact F2/F3 twist tomography. A prereveal N130 child carrying boundary x F3 characters is higher information than enlarging the old block.
...
- **Semantic control plane:** P50/P48 wrappers are on main; open #347 composes the live corpus to 44 paths, 13 direct, 12 covered, 18 migrations and 1 utility. PR #339 remains a synthetic-only odd-jet primitive and PR #342 a resource gate. None is new scientific evidence. Migration changes future composability, not historical evidence role; the next layer must record units, model/harmonic, size map, chronology and dependencies.
