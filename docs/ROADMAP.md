...
Main also has a coherent 200-descriptor/952-map registry. Through `8ba849c`, the live corpus contains 51 paths: 20 direct typed entrypoints, 19 wrapper-covered frozen kernels, 11 migrations and 1 utility. The seven newest entrypoints cover six existing scorer families without modifying any frozen kernel, numerical result or scientific conclusion. This is a semantic transport layer, not a new evidence grade; topology typing does not yet type every derived response. `Migration_required` does not invalidate a result.
...
Merged PR #341 types the C4 holdout, PR #344 types both Issue43 kernels, and the later wrappers through PR #357 type six more scorer families; none changes data, score or scientific priority. The exact parametric, log-odds and Pell artifacts are bounded method or regression contracts rather than physical evidence. The underlying model chronologies remain what they were, while these semantic gates are post-score and create no evidence votes.
...
- **Semantic control plane:** main through `8ba849c` inventories 51 paths: 20 direct, 19 covered, 11 migrations and 1 utility. PRs #339/#349/#354 are synthetic exact primitives and PR #351 is a bounded exact regression; PR #342 remains a resource gate. None is new scientific evidence. Migration changes future composability, not historical evidence role; the next layer must record units, model/harmonic, size map, chronology, response semantics and dependencies.
