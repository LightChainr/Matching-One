...
## Next attention — #537 full thermal gate and separate fixed-m phase control
...
The [exact gate audit](https://github.com/LightChainr/Matching-One/blob/ec3941b03b2694e827db1cba34766a82e6146a5a/notes/p337-thermal-pivotal-gate-audit.md) has already resolved the two-channel algebra: `d_p Cov(O,a)` is the sum of kernel-reconnection and original rank/readout-pivotal channels. Bell-8 checks 4,140 partitions and 64,954 joins (`29,970` nonzero; `9,952` positive; `20,018` negative; `max |16 Delta g|=68`), and N13 checks 1,198,080 state-edge-pair records with 312 same-shared-count sign flips. In N9/N10/N13 controls the omitted readout term reverses the sign. The single-channel `partial_p E[g_xy]` shortcut is therefore retired.

The asymptotic question remains the full original-U quantity `T_N=jY_p-R*jM_p-R_p*jM`: prove `T_N=o(D/A_N)` and transport exact-`p_c` control to the pooled root. A naive absolute `R^4*pi_4(R)^3` shell bound needs `alpha_4>4/3`, beyond the currently available rigorous input of some `alpha_4>1`. The second, separate fixed-m question still needs restricted-sector/phase control. The [actual component review](https://github.com/LightChainr/Matching-One/blob/baa5d33b2f87b2868aa0cb9d3f6518c93dbf3bff/notes/p337-fixed-m-relative-bound.md) already shows that improving the bare nonnegative KP counting constant is not the task.

The full gate is exactly `partial_u partial_epsilon Yhat=T_N/D=J_N/A_N` in the matching-mean coordinate, equivalently one signed Schur covariance in logit coordinates. The [finite scope decision](https://github.com/LightChainr/Matching-One/blob/8d228aa4f9de73821d1f0f8ad4f1c3b5a5205024/notes/p537-finite-rank-one-decision-20260901.md) absorbs the nonzero preflight, #544 N25 collar and #545 all-scale pointwise witnesses while recording their semantic and root-conditioning limits. Finite pure-thermal rank-one rescue is stopped; #544/#545 are closed unmerged assets. The next action is the one-defect diagonal-edge falsifier on the full pooled-root signed-mass graph. If one edge changes both slow variables with nonzero Schur weight, stop the two-defect/high-arm route. Only no diagonal edge plus separable annuli permits a six-arm bound. Do not open N expansion, random production or another finite-minor branch first.

The post-cleanup Drafts #530-#533 and issues #539/#542 are P2 support/candidates, not competing priorities. #530/#531 provide oracle/method diagnostics on the existing exact N25 block; #532 preserves finite two-bridge algebra. At #533 head `5aa929a6ef4ff92d71f6f36097b763c05ad71ab8`, C1 is restricted to the relaxed all-`w` Catalan/Toeplitz one-particle identity, the immediate `EW/WE` retrace rule, a legal nonlocal one-west counterexample, and a `pq` lemma conditional on an independently established exponential two-gap kernel. The actual physical beta-cloud/collar, physical `w>=2` endpoint, rank-one common-coordinate step, second thermal moments, pooled root/original-U denominator and uniform contour/sector remainder remain C0. `eed2190c` is now in ancestry, but `410015f5` and `2690f665` remain absent; the `a*s`, control-character and adjacent-hole-order defects remain. #542 stays open for those bounded proof/counterexample checks; it is not a production queue and does not displace #537's one-defect diagonal-edge gate.

**P337 finite transmission and critical spatial boundary are now closed at their declared levels.** [Homogeneous N50](https://github.com/LightChainr/Matching-One/blob/ef3b2c68f824e29421747c805ea7a505aca41908/experiments/p337-homogeneous-n50-20260831/RESULT.md) gives original U=1.0615603877 and V_S=+0.0543457827. The [canonical local pair](https://github.com/LightChainr/Matching-One/blob/2ba8863f75e0ced211b7b5442e8cddbe2fbd3deb/notes/regular-pair-interaction-result.md) has direct Q1 response zero and fixed mixed response -0.04503611398. The [fresh spatial blocks](https://github.com/LightChainr/Matching-One/blob/a237968f1d7a82d26b46e83c58179dbba7f1a908/notes/regular-pair-spatial-transmission-result.md) reject finite zero transmission at L64/distance16 without fitting an exponent. The [complete N25 joint response](https://github.com/LightChainr/Matching-One/blob/410015f5505dc2d8ca0e9ac904f656a4adc9fe86/notes/regular-pair-joint-transmission-result.md) is J2=-0.0055194314248394015; both adjacent and nonadjacent parts are strictly negative, rejecting additive-linear and NN-contact-only closures. Finally, the [critical square-site theorem](https://github.com/LightChainr/Matching-One/blob/eed2190c04b67084ab5aef5827e00377853a0bca/notes/p337-critical-spatial-summability.md) proves E_pc|g_xy|<=C d^(-2-eta) for some eta>0, hence a uniformly summable raw spatial kernel and vanishing macroscopic tail. It does not bound full U or apply automatically to the finite p_ref/root. Stop distance grids, exponent fits, completion scans and uniform-J2 repeats. #537 now owns the full two-channel thermal/pivotal theory gate, alongside the separate fixed-m phase/sector control; random production remains empty.
...
- bounding only the kernel-reconnection channel while omitting original rank/readout pivotals from the covariance derivative;
- maintaining branch-local NEXT lists as parallel priority systems.

## Decision already delivered

The independent blocks have removed the specified strong transmission/closure candidates. Preserve that stop and the current C3 phenomenology boundary. The only active exception is the non-sampling #537 theorem gate; failed templates do not generate a replacement production queue.
