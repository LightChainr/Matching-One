...
class HermiteKrawtchoukScalingJetTests(unittest.TestCase):
    ...
    def test_finite_jet_is_primary_and_differs_by_known_falling_factor(self) -> None:
        ...
        asymptotic = asymptotic_scaling_derivative_jet(coefficients, n, p0, alpha)
        for order, (finite_value, asymptotic_value) in enumerate(zip(finite, asymptotic)):
            self.assertLess(
                abs(finite_value - finite_jet_factor(n, order) * asymptotic_value),
                mp.mpf("1e-52"),
            )
    ...
    def test_width_cross_residual_cancels_width_and_amplitude(self) -> None:
        ...
        parent = [parent_amplitude * value / parent_width**order for order, value in enumerate(shape)]
        child = [child_amplitude * value / child_width**order for order, value in enumerate(shape)]
        residual = width_cross_residual(parent, child, parent_width, child_width)
    ...
    def test_q2_and_jordan_cocycles(self) -> None:
        ...
        self.assertTrue(all(abs(value) < mp.mpf("1e-48") for value in cocycle_residual(q2, q2_2, q2_5, mp.mpf(8) / 5)))
        ...
        self.assertTrue(
            all(
                ...
                for value in cocycle_residual(jordan, jordan_2, jordan_5, mp.log(5) / mp.log(2))
            )
        )
    ...
    def test_exact_rank_gap_is_canonical_neutral_window_area(self) -> None:
        for n in range(13):
            for occupation in range(n + 1):
                self.assertEqual(bernstein_basis_integral(n, occupation), Fraction(1, n + 1))
            for k_minus in range(n + 2):
                for k_plus in range(k_minus, n + 2):
                    signs = [microcanonical_matching_sign(n, occupation, k_minus, k_plus) for occupation in range(n + 1)]
                    self.assertEqual(sum(1 - sign**2 for sign in signs), k_plus - k_minus)
                    self.assertEqual(canonical_neutral_window_area(n, k_minus, k_plus), Fraction(k_plus - k_minus, n + 1))
    ...
    def test_rank_surrogate_contains_known_one_plus_inverse_n_jacobian(self) -> None:
        ...
        for n, mean_gap in ((17, mp.mpf("5.25")), (185, mp.mpf("11.75"))):
            ...
            self.assertLess(abs(surrogate - (1 + mp.mpf(1) / n) * canonical), mp.mpf("1e-55"))
    ...
    def test_prediction_artifact_freezes_width_first(self) -> None:
        ...
        self.assertEqual(artifact["rank_gap_bridge"]["exact_canonical_neutral_area"]["status"], "exact")
        self.assertEqual(artifact["rank_gap_bridge"]["primary_width"], "w_can(N)=N^(3/8)*E[G]/(N+1)")
        self.assertEqual(artifact["thermal_scaling_jet"]["primary_coordinate"], "exact_finite_N_jet")
        self.assertEqual(artifact["thermal_scaling_jet"]["finite_to_asymptotic_factor"], "sqrt((N)_r/N^r)")
        width_residual = artifact["frozen_predictions"]["rank_gap_width_collapse"]["division_free_residual"]
