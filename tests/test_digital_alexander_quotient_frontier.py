...
class DigitalAlexanderQuotientFrontierTests(unittest.TestCase):
    ...
    def test_all_hnf_representatives_and_permutations_are_exhausted(self) -> None:
        self.assertEqual(self.artifact["status"], "no_counterexample_through_index_13")
        self.assertEqual(self.artifact["HNF_representatives"], 140)
        self.assertEqual(self.artifact["filtration_paths"], 101140028118)
        self.assertEqual(len(self.artifact["geometries"]), 140)
    ...
    def test_face_degeneracy_partition_is_complete(self) -> None:
        ...
        self.assertEqual(self.artifact["honest_face_representatives"], 93)
        self.assertEqual(self.artifact["self_identifying_face_representatives"], 47)
    ...
    def test_index_thirteen_extension_is_complete(self) -> None:
        rows = [row for row in self.artifact["geometries"] if row["order"] == 13]
        self.assertEqual(len(rows), 14)
        self.assertTrue(all(row["permutations"] == 6227020800 for row in rows))
        self.assertEqual(sum(row["four_distinct_face_corners"] for row in rows), 10)
        self.assertEqual(sum(row["rank_one_plateau_steps"] for row in rows), 447056064000)
    ...
    def test_index_and_plateau_diagnostics_are_frozen(self) -> None:
        ...
        self.assertEqual(self.artifact["rank_one_plateau_steps"], 500805335024)
        self.assertEqual(self.artifact["cached_subsets"], 285020)
        ...
        self.assertEqual(sum(row["rank_one_plateau_steps"] for row in geometries), 500805335024)
        self.assertEqual(sum(row["cached_subsets"] for row in geometries), 285020)
