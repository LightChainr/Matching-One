...
class ScorerDescriptorAdoptionAuditTests(unittest.TestCase):
    ...
    def test_checked_result_has_current_partition(self) -> None:
        ...
        self.assertEqual(result["counts"]["total"], 63)
        ...
        self.assertEqual(len(result["rows"]), 63)
        ...
        self.assertEqual(
            statuses["scripts/threshold_score_modes.py"],
            "covered_frozen_kernel",
        )
        self.assertEqual(
            statuses["scripts/threshold_score_modes_typed.py"],
            "direct_typed_entrypoint",
        )
        self.assertEqual(
