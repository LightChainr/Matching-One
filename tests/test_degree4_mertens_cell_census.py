import copy
import json
from pathlib import Path
import sys
import unittest

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "scripts"))

from degree4_interval_exclusion import build_result, output_path, validate_result  # noqa: E402


INTERVAL_ID = "mertens-2022-p-cell"


class DegreeFourMertensCellCensusTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.result = build_result(INTERVAL_ID)

    def test_committed_result_reproduces(self):
        checked = json.loads(output_path(INTERVAL_ID).read_text())
        self.assertEqual(checked, self.result)
        self.assertFalse(validate_result(checked, INTERVAL_ID)["excluded"])

    def test_fifteen_quartics_survive_the_method_interval(self):
        row = self.result["interval_result"]
        self.assertEqual(row["near_candidates_exactly_checked"], 1660)
        self.assertEqual(row["root_filter_candidates"], 127)
        self.assertEqual(row["root_containing_polynomials"], 15)
        self.assertEqual(row["distinct_roots_in_interval"], 15)
        self.assertEqual(row["near_candidates_with_stationary_point"], 0)
        self.assertEqual(len(row["root_witnesses"]), 15)
        self.assertEqual(row["closest_polynomial"]["coefficients_ascending"], [-97, 87, 54, 98, 49])

    def test_tampering_fails_closed(self):
        changed = copy.deepcopy(self.result)
        changed["interval_result"]["distinct_roots_in_interval"] = 14
        with self.assertRaisesRegex(ValueError, "does not exactly reproduce"):
            validate_result(changed, INTERVAL_ID)


if __name__ == "__main__":
    unittest.main()
