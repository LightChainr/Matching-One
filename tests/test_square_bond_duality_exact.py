...
# Locked even-sector expectations at p=1/2.  These are C5 rationals from
# exhaustive enumeration, not fitted amplitudes.
...
class SquareBondDualityExactTests(unittest.TestCase):
    ...
    def test_l2_naive_complement_is_not_the_duality_map(self) -> None:
        result = enumerate_exact(2)
        # Bit-complement occupies vacant primal edges, not identified dual
        # edges.  Treating it as the duality involution is a convention error.
        self.assertGreater(result["naive_complement_swap_failures"], 0)
