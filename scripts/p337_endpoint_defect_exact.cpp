// Fixed single-A-vacancy parent N50: enumerate only the 25 free B sites.
// Rollback union-find and Gaussian quotient key follow d2cec6a7.
// No saturated baseline, child-lattice assumption, Monte Carlo, or response score.
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdint>
...
class Enumerator {
    ...
    void visit(int position, int k, int edges, int faces) {
        if (position == free_n) {
            ...
            for (int j=0; j<6; ++j) sums[k][j] += row[j];
            return;
        }
        ...
        for (int u : white_neighbors[v]) if (colour[u] == 0) white.join(v, u);
...
int main(int argc, char** argv) {
    try {
        if (argc!=4) throw std::invalid_argument("usage: endpoint-defect a b output.csv");
