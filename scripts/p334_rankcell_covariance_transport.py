        ...
        for ori in ORIS:
            ...
            for cell in CELLS[1:]:
                ...
                for (i, j), pair in zip(PAIRS, PAIR_NAMES):
                    ...
                    cell_total.append(d[f"{cell}.H.{mark}.{ori}.{pair}"]-group_product)
                for part, values in (("within_rankcell_prefixes", within), ("between_rankcells", between),
                                     ("within_rankcell_total", cell_total)):
            ...
            for field in shape_coordinates(0, 0, 0):
                for part in ("within_rankcell_prefixes", "between_rankcells", "within_rankcell_total"):
...
def main():
    ...
    # This derived file is regenerated on a versioned analysis branch; its
    # first two-way release remains available in git history.
    ...
    result = {"schema": "matching-one/p334-rankcell-covariance-transport/v1",
              ...
              "identity": "between_prefixes=within_rankcell_prefixes+between_rankcells; total=within_rankcell_total+between_rankcells",
