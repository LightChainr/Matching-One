...
Issue #118 asks for dimensionless combinations *before* any new production.
The construction uses only scaling dimensions already frozen in the P48
...
Raw A_T / A_I does **not** cancel two independent lattice couplings and is
not a universality candidate.

Descriptive reconstructions from committed P48 scaled amplitudes are
development-only.  They are not a frozen target for issue #57.
"""
...
P35_CLOSURE = (
    ...
    / "results"
    / "server-20260828"
    / "P35-amplitude-closure"
    / "closure.csv"
)

# Leading N exponents of the unscaled observables.
...
def ratio_exponent(numerator: str, denominator: Tuple[str, ...]) -> Fraction:
    """Return the net N-power of a monomial in the named observables."""
...
def p35_last_B(path: Path = P35_CLOSURE) -> Tuple[int, float]:
    """Return (N, B) from the last committed P35 closure row.

    This is an explicit common-metric approximation, not a jackknife B at
    N=185/265.  Callers must label it as such.
    """
