...
class Enumerator {
    ...
    JointSum joint_sum() {
        ...
        for (int y = 1; y < n; ++y) if (colour[y] == 0) {
            ...
            const std::int64_t value = kernel.g16[key];
            sum.total += value;
...
public:
    Enumerator(int aa, int bb, const Kernel& table) : kernel(table), a(aa), b(bb) {
        if (!((a == 5 && b == 0) || (a == 4 && b == 3)))
...
int main(int argc, char** argv) {
    try {
        if (argc != 5)
            throw std::invalid_argument("usage: regular_pair_joint_u_exact a b kernel.tsv output.csv");
        const auto aa = integer(argv[1]), bb = integer(argv[2]);
        if (!((aa == 5 && bb == 0) || (aa == 4 && bb == 3)))
            throw std::invalid_argument("fixed N=25 pair is (5,0),(4,3)");
        const auto start = std::chrono::steady_clock::now();
        const Kernel kernel = read_kernel(argv[3]);
        Enumerator(static_cast<int>(aa), static_cast<int>(bb), kernel).run(argv[4]);
