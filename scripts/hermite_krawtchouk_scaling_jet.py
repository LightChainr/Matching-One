...
def finite_jet_factor(n: int, order: int) -> mp.mpf:
    """Return ``d_fin/d_asym=sqrt((N)_r/N^r)``."""
...
def width_normalized_jet(
    ...
    return [value * canonical_width**order for order, value in enumerate(jet)]
...
def bernstein_basis_integral(n: int, occupation: int) -> Fraction:
    """Integrate a degree-N Bernstein basis element exactly on [0,1]."""
    if n < 0 or not 0 <= occupation <= n:
        raise ValueError("occupation must lie in 0..n")
    return Fraction(
...
def main() -> int:
    ...
    payload = {
        ...
        "rank_width_surrogate": (
            "w_rank(N)=N^(-5/8) E[K_plus-K_minus]=(1+1/N) w_can(N)"
        ),
        "exact_neutral_area_bridge": (
            "integral_0^1 E[U_{K~Bin(N,p)}] dp=E[K_plus-K_minus]/(N+1)"
        ),
    }
