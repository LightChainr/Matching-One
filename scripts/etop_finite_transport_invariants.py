...
def odd_sign_certificate(contract):
    ...
    return {"common_positive_scale": f"625/(1152*{contract['samples_per_shape_pair']})",
            ...
            "curves": certificates,
            "consequence": "These empirical A curves admit a unique interior-regular cumulative-quantile map after area normalization. Endpoint derivatives may degenerate. This proves neither population-wide positivity nor a finite physical coordinate identification."}
...
def main():
    ...
    result = {"status": "post-reveal finite common-transport analysis; zero new samples",
        "source_commit": "7b30648", "source_path": str((SOURCE/"score.json").relative_to(ROOT)),
        ...
        "empirical_odd_sign_certificate": odd_sign_certificate(source["contract"]),
    ...
    (OUT/"invariants.json").write_text(json.dumps(result, indent=2, allow_nan=False)+"\n")
    ...
    lines += ["", f"Joint: chi-square={score['chi_square']:.8g}/{score['df']}, nominal p={score['nominal_p_value']:.6g}.", "",
        ...
        f"The six finite-polynomial integrals use {nodes}-node Gauss-Legendre (degree at most{max_m*(n+1)+n}), with all source and target uncertainty propagated by deleting each of200 aligned batches. No new sampling is used.", "",
