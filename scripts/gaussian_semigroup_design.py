...
The square-torus orientation represented by ``a+i b`` has ``N=a^2+b^2``.
For reflection-even square-lattice harmonics, ``cos(4m theta)`` is an exact
rational function of ``a,b``. Multiplying both members of an orientation pair
by a Gaussian integer gives a parameter-free parent/child design for a proposed
spin/radial sector.

This script generates design metadata only. It never reads percolation
outcomes and is therefore safe to use for preregistration.
"""
...
class Gaussian:
    ...
    def canonical_d4(self) -> "Gaussian":
        """Return the first-octant representative under square D4 symmetry."""
    ...
    def cos4m(self, m: int) -> Fraction:
        """Return cos(4*m*theta) exactly using a Chebyshev recurrence."""
        if m < 0:
...
def default_catalog() -> Dict[str, object]:
    ...
    doubling_lineages = {
        "65_to_130": lineage_payload(
            Gaussian(8, 1), Gaussian(7, 4), doubling
        ),
        "85_to_170": lineage_payload(
            Gaussian(9, 2), Gaussian(7, 6), doubling
        ),
        "145_to_290": lineage_payload(
            Gaussian(12, 1), Gaussian(9, 8), doubling
        ),
    }
