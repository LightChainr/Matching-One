...
survives an exactly self-dual square model.  Before any Gaussian production
run, the primal/dual wrapping channels and the even/odd combinations must be
fixed by exhaustive identities.
...
P4[D'].  Those remain the production content of #42.
...
def wrapping_from_union_find(union_find: HomologyUnionFind) -> WrappingChannels:
    """Aggregate every union-find root into a wrapping-channel record."""
...
def primal_dual_wrapping(
    ...
    """Return primal open-bond wrapping and complementary dual wrapping."""
...
def geometric_dual_mask(mask: int, pairs: Sequence[BondPair]) -> int:
    """Transport dual occupation onto the identified primal edge set.

    Vacant primal bond i occupies the unique primal bond whose displaced
    edge equals ``pairs[i].dual``.  This is the geometric dual, not bit
    complement of ``mask``.
    """
...
def enumerate_exact(length: int) -> dict[str, object]:
    """Exhaustively enumerate a square-bond torus and return exact identities."""
    ...
    for mask in range(configuration_count):
        ...
        if (
            dual_bits != transported_primal_bits
            or primal_bits != transported_dual_bits
        ):
            dual_transport_failures += 1
        for name in CHANNELS:
            r_p = primal_bits[name]
            r_d = dual_bits[name]
            even = r_p + r_d  # 2S as an integer
            odd = r_p - r_d
            transported_even = (
                transported_primal_bits[name] + transported_dual_bits[name]
            )
            transported_odd = (
                transported_primal_bits[name] - transported_dual_bits[name]
            )
