// Full (k,g,q,count) histogram for the frozen N25 finite-coupling experiment.
// Geometry, rollback topology and traversal are unchanged from
// b70dc4bd:scripts/p337_closed_source_exact.cpp; only leaf statistics/output differ.
// g=2N+1-k-Sstar. Sparse signed-g keys impose no guessed maximum range.
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdint>
...
class Enumerator {
    ...
    std::array<std::map<int, std::array<std::uint64_t, 3>>, n + 1> histogram;
...
public:
    ...
    void run(const char* path) {
        ...
        out << "k,g,q,count\n";
        for (int k=0;k<=n;++k) for (const auto& [g, counts] : histogram[k]) {
            for (int q=-1;q<=1;++q) if (counts[q+1])
                out << k << ',' << g << ',' << q << ',' << counts[q+1] << '\n';
        }
