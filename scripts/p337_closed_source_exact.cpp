// Exact closed-source Sstar/Bvac enumeration on the fixed N25 Gaussian pair.
// Geometry and traversal inherited unchanged from 2bfe9b90.
...
class Enumerator {
    ...
    void visit(int v, int k, int edges, int faces) {
        if (v == n) {
            ...
            const Row row{1, q, e, sstar, bv, q*sstar, e*sstar, q*bv, e*bv};
...
public:
    ...
    void run(const char* path) {
        ...
        out << "k,count,sum_q,sum_e,sum_sstar,sum_bv,sum_qsstar,sum_esstar,sum_qbv,sum_ebv\n";
        for (int k=0;k<=n;++k) {
