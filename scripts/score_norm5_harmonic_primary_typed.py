#!/usr/bin/env python3
"""Type-safe entrypoint for the frozen norm-5 H4/H12 primary scorer.

The 2026-08-28 scoring kernel is preserved unchanged.  This wrapper adds the
post-#134/#170 observable-semantic contract required by Issue #146, validates
an exact matching-odd `either -> cross` map, invokes the frozen kernel, and
records the descriptors/map in the generated score.
"""
...
import score_norm5_harmonic_primary as frozen_kernel
from wrapping_channels import ObservableDescriptor, map_observable


SEMANTIC_MANIFEST = "predictions/norm5_harmonic_semantic_gate_20260829.yaml"
...
def load_semantic_gate(root: Path) -> tuple[dict, ObservableDescriptor, ObservableDescriptor, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("norm-5 semantic manifest must be a mapping")
    if payload.get("status") != "semantic_gate_added_before_norm5_target_reveal":
        raise ValueError("norm-5 semantic gate status changed")
    ...
    if transform.scale != float(expected["scale"]) or transform.offset != float(expected["offset"]):
        raise ValueError("registered norm-5 observable map no longer matches the semantic manifest")
    if transform.scale != 1.0 or transform.offset != 0.0:
        raise ValueError("norm-5 primary raw matching-odd map must be exact identity scale")
...
def find_output_path(arguments: Sequence[str]) -> Path:
    ...
    raise ValueError("typed norm-5 scorer requires --output")
...
def annotate_output(
    ...
    if not isinstance(payload, dict):
        raise ValueError("frozen scorer output must be a JSON object")
