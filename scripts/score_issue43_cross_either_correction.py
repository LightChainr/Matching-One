#!/usr/bin/env python3
"""Correct the Issue #43 matching-even score for the exact cross/either map.

The frozen source amplitude in ``two_spin4_heldout_20260828.yaml`` was fitted
from the P31 ``either/even`` channel.  The threshold-rank production engine,
however, stores rank-2 cross-wrapping thresholds, so the Issue #43 primary
``DeltaS`` observation is a cross-channel quantity.

For the complementary torus wrapping pair,

    S_either = 1 - S_cross,

therefore orientation contrasts satisfy

    DeltaS_cross = -DeltaS_either.

This scorer does exactly one operation to the frozen source prediction: negate
its DeltaS mean before scoring.  The source uncertainty, target observation,
exponent, amplitude magnitude, and all production data are unchanged.
"""
...
def corrected_score(primary: dict, prediction_text: str) -> dict:
    ...
    frozen_cross = [-x for x in frozen_either]
    ...
    return {
        ...
        "source_coefficient_se": source_se,
...
def main() -> int:
    ...
    result = corrected_score(primary, args.prediction.read_text(encoding="utf-8"))
