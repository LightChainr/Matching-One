// Prepared local four-port source counts. DO NOT RUN before the root contract
// and the two tensor gates are accepted and the coordinator gives GO.
// Geometry, white/Alexander topology, component rollback and subset traversal
// are retained from p337_closed_source_finite_exact.cpp. Only the leaf readout
// changes. All 2^25 configurations remain; the vacant-origin condition is a gate.
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdint>
...
struct Totals {
    ...
    std::int64_t q=0, e=0, twice_s=0, q_twice_s=0, e_twice_s=0;
};
...
class Enumerator {
    ...
    int twice_source() const {
        if (colour[0]!=0) return 0;
        for (int v:ports) if (colour[v]!=1) return 0;
        const int north=black.root(ports[0]), south=black.root(ports[1]);
        const int east=black.root(ports[2]), west=black.root(ports[3]);
        // Each case has exactly two distinct components with two ports each.
        // S=-t; opposite pairs have t=1, either adjacent pairing has t=1/2.
        if (north==south && east==west && north!=east) return -2;
        if (north==east && south==west && north!=south) return -1;
        if (north==west && south==east && north!=south) return -1;
        return 0;
    }
    ...
    void visit(int v,int k,int edges,int faces) {
        if (v==n) {
            ...
            const int e=q*q, s2=twice_source();
            auto& row=histogram[k];
            ++row.count; row.q+=q; row.e+=e; row.twice_s+=s2;
            row.q_twice_s+=q*s2; row.e_twice_s+=e*s2;
            return;
        }
        colour[v]=0;
    ...
    void run(const char* path) {
        ...
        for (int k=0;k<=n;++k) {
            ...
            out << k << ',' << row.count << ',' << row.q << ',' << row.e << ','
                << row.twice_s << ',' << row.q_twice_s << ',' << row.e_twice_s << '\n';
        }
