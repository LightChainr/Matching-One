...
def project(first: H, second: H, max_order: int) -> dict:
    ...
    # These are identities, not extra evidence: r=0 is the observable and
    # r=1 is its exact eta derivative in an orthonormal convention.
...
def analyze_size(data: Mapping[tuple[int, str, int], H], n: int, max_order: int) -> dict:
    ...
    # If z=(p-pc)N^(3/8), taking an r-th thermal response contributes
    # N^(3r/8), while the normalized binomial score removes N^(r/2).
    # Hence each score order advances the N-exponent by r/8.  Matching parity
    # selects the two interlaced towers below.
