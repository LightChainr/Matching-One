...
def main():
    ...
    files=('CONTRACT.md','score.py','produce-polynomial.py','producer/frontier-polynomial.cpp',name)
    hashes={}
    for file in files:
        path=PACKAGE/file
    ...
    with tempfile.TemporaryDirectory(prefix='p337-compiled-') as tmp:
        ...
        compile_command=['clang++','-O3','-std=c++17',str(PACKAGE/'producer/frontier-polynomial.cpp'),'-o',str(executable)]
        ...
        command=[str(executable),'--graph',str(PACKAGE/name),'--output-prefix',str(out/'population'),
                 '--authorization-commit',freeze,'--cpu-seconds','120','--rss-mib','4096','--state-cap','5000000']
