// Exact black-NN lifted frontier.  Independent C++17 port of the pinned Python.
...
Graph read_graph(const std::string& path) {
    ...
    // N<=64 bounds the count and even the conservative intermediate source sums
    // far below signed128.  Wider targets require a separately checked bound.
    if (g.n < 1 || g.n > 64) throw std::runtime_error("geometry outside N<=64 scope");
...
struct State {
    int r=0, hx=0, hy=0, k=0;
    std::array<int, MAX_FRONT> label{}, px{}, py{};
};

State decode(const std::string& key, int width) {
    if (key.size()<2) throw std::runtime_error("truncated state header");
    State s;
    std::size_t pos=0;
    s.r=static_cast<unsigned char>(key[pos++]);
    s.k=static_cast<unsigned char>(key[pos++]);
...
std::string encode(const State& s, const std::vector<int>& retained) {
    if (s.r<0 || s.r>2 || s.k<0 || s.k>64) throw std::runtime_error("invalid state header");
    ...
    std::string key;
    key.reserve(2+(s.r==1 ? 4:0)+retained.size()*(s.r==0 ? 5 : s.r==1 ? 3:1));
    key.push_back(static_cast<char>(s.r)); key.push_back(static_cast<char>(s.k));
...
struct Value { I128 count=0, sum_s=0; };
...
Config arguments(int argc,char**argv) {
    ...
    if (c.graph.empty() || c.prefix.empty() || c.seconds<=0 || c.seconds>55 || c.rss<=0 || c.rss>2048 || !c.cap || c.cap>5000000)
        throw std::runtime_error("explicit graph/output and CPU<=55s, RSS<=2048MiB, states<=5e6 required");
    return c;
}
...
int run(const Config& config) {
    ...
    rlimit limit{};
    limit.rlim_cur=static_cast<rlim_t>(std::ceil(config.seconds)+2);
    limit.rlim_max=static_cast<rlim_t>(std::min(60.0,std::ceil(config.seconds)+5));
    if (setrlimit(RLIMIT_CPU,&limit)) throw std::runtime_error("cannot set hard CPU limit");
    const double start=cpu_seconds();
    const auto wall_start=std::chrono::steady_clock::now();
    auto guard=[&]() {
        if (cpu_signal || cpu_seconds()-start>config.seconds) throw Limit("cpu_gate");
    ...
    states.emplace(encode(State{},{}),Value{1,2*graph.n+1});
    std::size_t max_states=1, max_key_bytes=2, transitions=0;
        ...
        for (int v=0;v<max_layers;++v) {
            ...
            for (const auto& item:states) {
                ...
                for (int occupied=0;occupied<=1;++occupied) {
                    ...
                    auto inserted=following.try_emplace(std::move(key),Value{});
                    Value& value=inserted.first->second;
                    value.count+=item.second.count;
                    value.sum_s+=item.second.sum_s+I128(ds)*item.second.count;
                    ++transitions;
            ...
            I128 total=0;
            for (const auto& item:following) total+=item.second.count;
            if (total!=(I128(1)<<(v+1))) throw std::runtime_error("prefix mass != 2^layer");
            ...
            trace<<std::setprecision(12)<<"{\"layer\":"<<completed<<",\"frontier\":"<<layer.after
                 <<",\"states\":"<<states.size()<<",\"prefix_count\":\""<<decimal(total)
    ...
    I128 total=0;
    if (complete) {
        std::array<std::array<Value,3>,65> hist{};
        std::array<I128,65> totals{};
        for (const auto& item:states) {
            State s=decode(item.first,0);
            hist[s.k][s.r].count+=item.second.count;
            hist[s.k][s.r].sum_s+=item.second.sum_s;
            totals[s.k]+=item.second.count;
