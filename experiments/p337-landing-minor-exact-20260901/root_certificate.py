...
def build():
    ...
    T01,A01=matrix[(0,1)];T12,A12=matrix[(1,2)];det=sub(mul(T01,A12),mul(T12,A01))
    ...
    p=(lo+hi)/2
    return {'schema':'matching-one/p337-landing-root-certificate/v1','interval':[str(left),str(right)],'matching_root_count':roots(matching,left,right),'determinant_root_count':roots(det,left,right),'matching_signs':[str(evaluate(matching,left)),str(evaluate(matching,right))],'determinant_signs':[str(evaluate(det,left)),str(evaluate(det,right))],'root_midpoint_decimal':f'{float(p):.16g}','matrix_at_root_midpoint':{'T_01':f'{float(evaluate(T01,p)):.16g}','T_12':f'{float(evaluate(T12,p)):.16g}','A_01':f'{float(evaluate(A01,p)):.16g}','A_12':f'{float(evaluate(A12,p)):.16g}','determinant':f'{float(evaluate(det,p)):.16g}'},'determinant_coefficients':[str(x) for x in det]}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path);a=ap.parse_args();z=build();assert z['matching_root_count']==1 and z['determinant_root_count']==0 and all(F(x)<0 for x in z['determinant_signs']);text=json.dumps(z,indent=2)+'\n';print(text,end='')
    if a.out:
