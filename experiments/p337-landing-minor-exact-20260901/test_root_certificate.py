...
class RootCertificateTests(unittest.TestCase):
    ...
    def test_thermal_sum_has_no_root(self): self.assertEqual(self.result['thermal_sum_root_count'],0)
    def test_endpoint_signs(self): self.assertTrue(all(F(x)<0 for x in self.result['determinant_signs']+self.result['thermal_sum_signs']))
