// Exact true-site-flip landing sufficient statistics for Issue #537.
//
// Fix x=0 vacant and z=East(x).  Enumerate the other 23 sites, evaluate the
// same background with z=0 and z=1, and scan every further vacant source mark
// y.  Rows retain the thermal landing type; columns retain the z-fixed C4
// representative of y and the Bell-8 source transition.  Kernel values are
// stored as exact g16 integers (a=g16/16; the later ordered-pair N^-2 weight is
...
struct RowKey {
    ...
    std::uint8_t arm_mask = 0, alternating = 0, occupied_component_degree_branch = 0;
    std::uint8_t occupied_arm_components = 0;
    std::int8_t occupied_landing_id0 = -1, occupied_landing_id1 = -1;
    std::int8_t vacant_separator_id0 = -1, vacant_separator_id1 = -1;
    std::uint8_t extra_source_port_contact = 0, source_absent = 0;
    std::uint32_t bell0 = 0, bell1 = 0;
    bool operator==(const RowKey& x) const {
        return std::tie(dx,dy,k_minus,rank0,rank1,arm_mask,alternating,
                        occupied_component_degree_branch,occupied_arm_components,occupied_landing_id0,
                        occupied_landing_id1,vacant_separator_id0,vacant_separator_id1,
                        extra_source_port_contact,source_absent,bell0,bell1) ==
               std::tie(x.dx,x.dy,x.k_minus,x.rank0,x.rank1,x.arm_mask,x.alternating,
                        x.occupied_component_degree_branch,x.occupied_arm_components,x.occupied_landing_id0,
                        x.occupied_landing_id1,x.vacant_separator_id0,x.vacant_separator_id1,
                        x.extra_source_port_contact,x.source_absent,x.bell0,x.bell1);
    }
};
...
struct RowHash {
    std::size_t operator()(const RowKey& x) const {
        ...
        mix(x.rank0); mix(x.rank1); mix(x.arm_mask); mix(x.alternating);
        mix(x.occupied_component_degree_branch); mix(x.occupied_arm_components);
        mix(std::uint8_t(x.occupied_landing_id0)); mix(std::uint8_t(x.occupied_landing_id1));
        mix(std::uint8_t(x.vacant_separator_id0)); mix(std::uint8_t(x.vacant_separator_id1));
        mix(x.extra_source_port_contact); mix(x.source_absent); mix(x.bell0); mix(x.bell1);
        return std::size_t(h);
    }
};
...
class Enumerator {
    ...
    int occupied_arm_components(const State& state) const {
        std::array<int,4> roots{};
        int used = 0;
        for (int direction = 0; direction < 4; ++direction) {
            const int u = neighbors[z][direction];
            if (!occupied[u]) continue;
            const int root = state.occupied_root[u];
            if (std::find(roots.begin(), roots.begin()+used, root) == roots.begin()+used)
                roots[used++] = root;
        }
        return used;
    }

    bool occupied_component_degree_branch(const State& state) const {
        std::array<bool,N> landing_root{};
        for (int direction = 0; direction < 4; ++direction) {
            const int u = neighbors[z][direction];
            if (occupied[u]) landing_root[state.occupied_root[u]] = true;
        }
        for (int v = 0; v < N; ++v)
            if (occupied[v] && landing_root[state.occupied_root[v]] && state.occupied_degree[v] >= 3)
                return true;
        return false;
    }

    struct LandingIds {
        int occupied0 = -1, occupied1 = -1;
        int vacant0 = -1, vacant1 = -1;
        std::array<int,2> occupied_roots{{-1,-1}};
        int occupied_count = 0;
    };

    LandingIds landing_ids(const State& state, int arm_mask) const {
        LandingIds ids;
        if (!(arm_mask == 5 || arm_mask == 10)) return ids;
        // Landing separators live in the off-z cut graph: z is a terminal
        // hole, not a white vertex allowed to join the two separator arms.
        DSU cut_white;
        for (int v = 0; v < N; ++v) if (v != z && !occupied[v]) {
            for (int direction : {0,1}) {
                const int u = neighbors[v][direction];
                if (u != z && !occupied[u]) cut_white.join(v,u);
                const int d = diagonal[v][direction];
                if (d != z && !occupied[d]) cut_white.join(v,d);
            }
        }
        std::array<int,2> occupied_raw{}, vacant_raw{};
        int oi = 0, vi = 0;
        for (int direction = 0; direction < 4; ++direction) {
            const int u = neighbors[z][direction];
            if (occupied[u]) occupied_raw[oi++] = state.occupied_root[u];
            else vacant_raw[vi++] = cut_white.root(u);
        }
        if (oi != 2 || vi != 2) throw std::logic_error("alternating landing does not have two arms per colour");
        ids.occupied0 = 0; ids.occupied1 = occupied_raw[1] == occupied_raw[0] ? 0 : 1;
        ids.vacant0 = 0; ids.vacant1 = vacant_raw[1] == vacant_raw[0] ? 0 : 1;
        ids.occupied_roots = occupied_raw;
        ids.occupied_count = ids.occupied1 + 1;
        return ids;
    }

    bool source_extra_contact(int y, const State& state, const LandingIds& ids) const {
        if (ids.occupied_count == 0) return false;
        for (int center : {0,y}) for (int direction = 0; direction < 4; ++direction) {
            const int u = neighbors[center][direction];
            if (!occupied[u]) continue;
            const int root = state.occupied_root[u];
            for (int j = 0; j < ids.occupied_count; ++j)
                if (root == ids.occupied_roots[j]) return true;
        }
        return false;
    }
    ...
    void consume(std::uint64_t mask) {
        ...
        int arm_mask = 0;
        for (int direction = 0; direction < 4; ++direction)
            arm_mask |= int(occupied[neighbors[z][direction]]) << direction;
        const int arm_components = occupied_arm_components(state0);
        const bool branch = occupied_component_degree_branch(state0);
        const LandingIds ids = landing_ids(state0, arm_mask);
        ...
        for (int y = 1; y < N; ++y) {
            ...
            const bool extra = !absent && source_extra_contact(y, state0, ids);
            const RowKey key{
                ...
                std::uint8_t(arm_mask), std::uint8_t(arm_mask == 5 || arm_mask == 10),
                std::uint8_t(branch), std::uint8_t(arm_components),
                std::int8_t(ids.occupied0), std::int8_t(ids.occupied1),
                std::int8_t(ids.vacant0), std::int8_t(ids.vacant1),
                std::uint8_t(extra), std::uint8_t(absent), bell0, bell1
            };
            auto& total = rows[key];
            ++total.count;
...
public:
    Enumerator(int aa, int bb, const Kernel& table) : a(aa), b(bb), kernel(table) {
        ...
        for (std::size_t i = 0; i < representatives.size(); ++i) {
            ...
            for (const auto [dx,dy] : std::array<std::pair<int,int>,2>{{{1,0},{0,1}}}) {
                ...
                if (index[key] < 0) { index[key] = int(representatives.size()); representatives.emplace_back(x+dx,y+dy); }
            }
        }
        ...
        // z is fixed East of x.  This is the unique C4 gauge for the site flip.
        z = neighbors[0][1];
        for (int v = 0; v < N; ++v) if (v != 0 && v != z) free_sites.push_back(v);
        if (free_sites.size() != 23) throw std::logic_error("wrong free-site count");

        // Shortest quotient representative in the fixed-z (East) C4 frame.
        ...
        for (int x = -12; x <= 12; ++x) for (int y = -12; y <= 12; ++y) {
            ...
            if (!found[v] || candidate < best[v]) { found[v] = true; best[v] = candidate; coordinate[v] = {x,y}; }
        }
    ...
    void run(const std::string& output) {
        ...
        std::sort(ordered.begin(), ordered.end(), [](const auto& lhs, const auto& rhs) {
            ...
            return std::tie(x.k_minus,x.rank0,x.rank1,x.arm_mask,x.occupied_component_degree_branch,
                            x.occupied_arm_components,x.occupied_landing_id0,x.occupied_landing_id1,
                            x.vacant_separator_id0,x.vacant_separator_id1,x.extra_source_port_contact,
                            x.source_absent,x.dx,x.dy,x.bell0,x.bell1) <
                   std::tie(y.k_minus,y.rank0,y.rank1,y.arm_mask,y.occupied_component_degree_branch,
                            y.occupied_arm_components,y.occupied_landing_id0,y.occupied_landing_id1,
                            y.vacant_separator_id0,y.vacant_separator_id1,y.extra_source_port_contact,
                            y.source_absent,y.dx,y.dy,y.bell0,y.bell1);
        });
        std::ofstream out(output);
        if (!out) throw std::runtime_error("cannot create output");
        out << "k_minus,rank0,rank1,arm_mask,alternating_four_arm,occupied_component_degree_branch,"
               "occupied_arm_components,occupied_landing_id0,occupied_landing_id1,"
               "vacant_separator_id0,vacant_separator_id1,extra_source_port_contact,"
        ...
        for (const auto& [key,total] : ordered) {
            ...
            out << int(key.k_minus) << ',' << int(key.rank0) << ',' << int(key.rank1) << ','
                << int(key.arm_mask) << ',' << int(key.alternating) << ',' << int(key.occupied_component_degree_branch) << ','
                << int(key.occupied_arm_components) << ',' << int(key.occupied_landing_id0) << ','
                << int(key.occupied_landing_id1) << ',' << int(key.vacant_separator_id0) << ','
                << int(key.vacant_separator_id1) << ',' << int(key.extra_source_port_contact) << ','
                << int(key.source_absent) << ',' << int(key.dx) << ',' << int(key.dy) << ','
                << key.bell0 << ',' << key.bell1 << ',' << total.count << ','
                << total.sum_q0 << ',' << total.sum_e0 << ',' << total.sum_a16_0 << ','
                << total.sum_q0_a16_0 << ',' << total.sum_e0_a16_0 << ','
...
int main(int argc, char** argv) {
    try {
        if (argc != 5) throw std::invalid_argument("usage: siteflip_landing_exact a b kernel.tsv output.csv");
        ...
        std::cout << std::setprecision(17)
                  << "{\"status\":\"completed\",\"N\":25,\"a\":" << a << ",\"b\":" << b
                  << ",\"fixed_x\":0,\"fixed_z_direction\":\"E\",\"backgrounds\":" << BACKGROUNDS
                  << ",\"kernel_rows\":" << kernel.rows << ",\"a_scale_denominator\":16,"
                  << "\"fixed_x_pair_scale_denominator\":25,\"c4_fixed_z_orbit_multiplier\":4,"
                  << "\"source_absent_sentinel\":" << KEY_SPACE
